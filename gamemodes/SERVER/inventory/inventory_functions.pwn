...
FUNC::Inventory_Clear(playerid)
{
    ...
    return mysql_tquery(mdb_query, sprintf("DELETE FROM `inventory` WHERE `ID` = '%d'", AccountData[playerid][pID]));
}
...
FUNC::Inventory_SetQuantity(playerid, item[], quantity)
{
    ...
    if (itemid != -1)
    {
        mysql_tquery(mdb_query, sprintf("UPDATE `inventory` SET `invQuantity` = %d WHERE `ID` = '%d' AND `invID` = '%d'", quantity, AccountData[playerid][pID], InventoryData[playerid][itemid][invID]));
...
stock Inventory_Remove(playerid, item[], quantity = 1)
{
	...
	if (itemid != -1)
	{
		...
		if (quantity == -1 || InventoryData[playerid][itemid][invQuantity] < 1)
		{
	        ...
	        mysql_tquery(mdb_query, sprintf("DELETE FROM `inventory` WHERE `ID` = '%d' AND `invID` = '%d'", AccountData[playerid][pID], InventoryData[playerid][itemid][invID]));
		}
		else if (quantity != -1 && InventoryData[playerid][itemid][invQuantity] > 0)
		{
            mysql_tquery(mdb_query, sprintf("UPDATE `inventory` SET `invQuantity` = `invQuantity` - %d WHERE `ID` = '%d' AND `invID` = '%d'", quantity, AccountData[playerid][pID], InventoryData[playerid][itemid][invID]));
		}
		return 1;
	}
	return 0;
...
stock Inventory_Add(playerid, item[], model, quantity = 1)
{
	...
	if (itemid == -1)
	{
	    ...
	    if (itemid != -1)
	    {
            ...
            mysql_tquery(mdb_query, sprintf("INSERT INTO `inventory` (`ID`, `invItem`, `invModel`, `invQuantity`) VALUES('%d', '%s', '%d', '%d')", AccountData[playerid][pID], item, model, quantity),"OnInventoryAdd", "dd", playerid, itemid);
	        return itemid;
		}
		return -1;
	}
	else
	{
	    ...
	    InventoryData[playerid][itemid][invQuantity] += quantity;
        mysql_tquery(mdb_query, sprintf("UPDATE player_characters SET Char_BackpackWeight = '%.3f' WHERE pID = '%d'", AccountData[playerid][pBeratItem], AccountData[playerid][pID]));
...
public OnPlayerUseItem(playerid, itemid, itemname[])
{
    ...
    else if(!strcmp(itemname, "ClipPistol"))
	{
		...
		if(weaponid >= 22 && weaponid <= 24) //PISTOL
		{
			...
			{
				GivePlayerWeaponEx(playerid, weaponid, ammo+8);
			}
			else
			{
				GivePlayerWeaponEx(playerid, weaponid, ammo+4);
			}
			AccountData[playerid][pClipP] -= 1;
			Inventory_Remove(playerid, "ClipPistol", 1);
            GivePlayerWeaponEx(playerid, weaponid, ammo+45);
    ...
    else if(!strcmp(itemname, "ClipSG"))
	{
		...
		if(weaponid < 25 || weaponid > 3272)
			return Error(playerid, "this weapon does not support to use clip");

		if(weaponid >= 25 && weaponid <= 27) //Rifle
		{
			Inventory_Remove(playerid, "ClipSG", 1);
			GivePlayerWeaponEx(playerid, weaponid, ammo+90);
			Info(playerid, "Succes reloaded ammo weapon %s with Clip", ReturnWeaponName(weaponid));
    ...
    {
        if(AccountData[playerid][ActivityTime] != 0) return ShowTDN(playerid, NOTIFICATION_WARNING, "Anda sedang melakukan sesuatu, harap tunggu!");
        if(AccountData[playerid][pFamily] == -1) return ShowTDN(playerid, NOTIFICATION_ERROR, "Hanya anggota Family yang dapat menggunakan ini!");
        new Float:AM;
    ...
    else if(!strcmp(itemname, "Armour", true))
    {
        if(AccountData[playerid][ActivityTime] != 0) return ShowTDN(playerid, NOTIFICATION_WARNING, "Anda sedang melakukan sesuatu, harap tunggu!");
        if(AccountData[playerid][pFaction] != FACTION_POLISI) return ShowTDN(playerid, NOTIFICATION_ERROR, "Hanya anggota kepolisian yang dapat menggunakan ini!");
        new Float:AM;
        GetPlayerArmour(playerid, AM);
        if(AM >= 95) return ShowTDN(playerid, NOTIFICATION_ERROR, "Armour anda masih di 95%, tidak dapat menggunakan lagi!");

        pUseItemTimer[playerid] = SetTimerEx("UseArmour", 1000, true, "i", playerid);
        PlayerTextDrawSetString(playerid, ProgressBar[playerid][3], "MEMAKAI ARMOUR");
        ShowProgressBar(playerid);
        
        AccountData[playerid][ActivityTime] = 1;
        ApplyAnimationEx(playerid, "COP_AMBIENT", "Copbrowse_loop", 4.1, 1, 0, 0, 0, 0, 1);
        SendRPMeAboveHead(playerid, "Mengambil Armour lalu menggunakannya", X11_PLUM1);
        ShowItemBox(playerid, "Removed 1x", "Armour", 19515);
        Inventory_Close(playerid);
    }    
    else if(!strcmp(itemname, "Rokok", true))
    {
        ...
        SendClientMessageEx(playerid, -1, ""WHITE"[i] Gunakan "GREEN"`ALT`"WHITE" untuk menghisap rokok "YELLOW"`/stopsmoke'`"WHITE" untuk membuang rokok");
        Inventory_Close(playerid);
...
public OpeningBackpack(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 115)
    {
        ...
        Vehicle_Create(playerid, 468, FACTION_NONE, POS[0], POS[1], POS[2], POS[3], 11, 11, 10000);
    }
    else 
    {
...
forward UseArmour(playerid);
public UseArmour(playerid)
{
	if(!IsPlayerConnected(playerid))
	{
		KillTimer(pUseItemTimer[playerid]);
	...
	if(AccountData[playerid][ActivityTime] >= 3)
	{
        ...
        SetPlayerArmourEx(playerid, Armour);
		Inventory_Remove(playerid, "Armour");
        ShowItemBox(playerid, "Removed 1x", "Armour", 19515);
	}
	else
	{
