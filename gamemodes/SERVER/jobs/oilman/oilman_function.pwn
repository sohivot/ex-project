    ...
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(AccountData[playerid][pJob] == JOB_OILMAN)
        {
            if(Oilman_CollectArea(playerid))
            {
                ...
                if(Inventory_Count(playerid, "Petrol") >= 100) return ShowTDN(playerid, NOTIFICATION_ERROR, "Petrol Kamu Di Tas Sudah Banyak!");
            ...
            {
                if(AccountData[playerid][ActivityTime] != 0) return ShowTDN(playerid, NOTIFICATION_WARNING, "Anda sedang melakukan sesuatu, harap tunggu!");
                if(Inventory_Count(playerid, "Pure oil") >= 100) return ShowTDN(playerid, NOTIFICATION_ERROR, "Pure oil Kamu Di Tas Sudah Banyak!");
                if(IsPlayerInjured(playerid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda sedang pingsan!");
            ...
            {
                if(AccountData[playerid][ActivityTime] != 0) return ShowTDN(playerid, NOTIFICATION_WARNING, "Anda sedang melakukan sesuatu, harap tunggu!");
                if(Inventory_Count(playerid, "GAS") >= 100) return ShowTDN(playerid, NOTIFICATION_ERROR, "Gas Kamu Di Tas Sudah Banyak!");
                if(IsPlayerInjured(playerid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda sedang pingsan!");
...
public OilMixxing(playerid)
{
    ...
    if(GetTotalWeightFloat(playerid) > 100)
    {
    ...
    if(AccountData[playerid][ActivityTime] >= 120)
    {
        ...
        Inventory_Add(playerid, "GAS", 1650, 5);
        ShowItemBox(playerid, "Removed 1x", "Pure oil", 3632);
        ShowItemBox(playerid, "Received 5x", "GAS", 1650);
...
public OilRefined(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 150)
    {
        ...
        Inventory_Remove(playerid, "Petrol", 5);
        Inventory_Add(playerid, "Pure Oil", 3632, 1);
        ShowItemBox(playerid, "Removed 5x", "Petrol", 19621);
        ShowItemBox(playerid, "Received 1x", "Pure Oil", 3632);
        GivePlayerXP(playerid, DEFAULT_XP);
    }
    else 
    {
...
public TakingOil(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 150)
    {
        ...
        Inventory_Add(playerid, "Petrol", 19621, 3);
        ShowItemBox(playerid, "Received 3x", "PETROL", 19621);
        GivePlayerXP(playerid, DEFAULT_XP);
    }
    else 
    {
