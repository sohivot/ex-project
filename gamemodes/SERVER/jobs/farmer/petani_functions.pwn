...
stock Weed_Delete(id)
{
    ...
    mysql_tquery(mdb_query, sprintf("DELETE FROM `weeds` WHERE `ID` = '%d'", WeedData[id][weedID]));
...
stock Weed_Create(playerid, type)
{
    ...
    if(GetPlayerPos(playerid, x, y, z))
    {
        forex(i, MAX_WEED) if(!WeedData[i][weedExists])
        {
            ...
            new query[178];
            mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `weeds` (`Time`) VALUES ('%d')", WeedData[i][weedTime]);
            mysql_tquery(mdb_query, query, "OnWeedCreated", "d", i);    
            return i;
        }
    }
    return -1;
...
{
    new query[596];
    mysql_format(mdb_query, query, sizeof(query), "UPDATE `weeds` SET ");
    mysql_format(mdb_query, query, sizeof(query), "%s`PosX`='%f', ", query, WeedData[id][weedPos][0]);
    mysql_format(mdb_query, query, sizeof(query), "%s`PosY`='%f', ", query, WeedData[id][weedPos][1]);
    mysql_format(mdb_query, query, sizeof(query), "%s`PosZ`='%f', ", query, WeedData[id][weedPos][2]);
    mysql_format(mdb_query, query, sizeof(query), "%s`Time`='%d', ", query, WeedData[id][weedTime]);
    mysql_format(mdb_query, query, sizeof(query), "%s`Type`='%d' ", query, WeedData[id][weedType]);
    mysql_format(mdb_query, query, sizeof(query), "%sWHERE `ID` = '%d'", query, WeedData[id][weedID]);
