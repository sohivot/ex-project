...
CMD:weapon(playerid, params[])
{
	...
	if(sscanf(params, "s[20]S()[128]", name, give))
		return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/weapon [give/drop/pickup/pos/bone/hide]");

	if(!strcmp(name, "pos", true))
	{
		if (!weaponid)
			return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak memegang senjata!");
		...
		if (EditingWeapon[playerid])
			return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda sudah mengedit senjata!");

		if (WeaponSettings[playerid][weaponid - 22][Hidden])
			return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak dapat menyesuaikan senjata tersembunyi!");
	...
	else if (!strcmp(name, "hide", true))
	{
		...
		if (EditingWeapon[playerid])
			return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak dapat menyembunyikan senjata saat Anda mengeditnya!");

		if (!IsWeaponHideable(weaponid))
			return ShowTDN(playerid, NOTIFICATION_ERROR, "Senjata ini tidak bisa disembunyikan!");
		...
		{
			format(string, sizeof(string), "Anda telah mengaturnya %s untuk menunjukkan!", weaponname);
		...
		else
		{
			...
			format(string, sizeof(string), "Anda telah mengaturnya %s untuk tidak ditampilkan!", weaponname);
			WeaponSettings[playerid][index][Hidden] = true;
		}
		SendClientMessage(playerid, -1, string);
	   
		mysql_format(mdb_query, string, sizeof(string), "INSERT INTO weaponsettings (Owner, WeaponID, Hidden) VALUES ('%d', %d, %d) ON DUPLICATE KEY UPDATE Hidden = VALUES(Hidden)", AccountData[playerid][pID], weaponid, WeaponSettings[playerid][index][Hidden]);
	...
	else if(!strcmp(name, "give", true))
	{
		...
		if (!weaponid)
			return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak memegang senjata!");
		new otherid;	
		if(sscanf(give, "ud", otherid, ammo))
			return ShowTDN(playerid, NOTIFICATION_ERROR, "/weapon [give] [playerid] [ammo]");

		if(ammo > GetPlayerAmmoEx(playerid))
        	return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak memiliki cukup amunisi!");
			
		if(otherid == INVALID_PLAYER_ID || otherid == playerid || !NearPlayer(playerid, otherid, 5.0))
			return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda harus berada di dekat pemain target!");

    	if(ammo < 1 || ammo > 300)
        	return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda telah menentukan amunisi senjata yang tidak valid, 1 - 300");
	...
	else ShowTDN(playerid, NOTIFICATION_ERROR, "Anda telah menentukan opsi yang tidak valid.");
	return 1;
}
/*
GetAmmoStatus(playerid, weaponid) {
	new typeammo[52], slot = g_aWeaponSlots[weaponid];
	if(AccountData[playerid][pGunType][g_aWeaponSlots[weaponid]] == 1) {
		if(AccountData[playerid][pGunType][g_aWeaponSlots[weaponid]]) {
			format(typeammo, sizeof(typeammo), "Ammo Surplus");
			//typeammo = "%d~n~SURPLUS", PlayerGuns[playerid][slot][pGunType];
		}
		else {
			typeammo = "~r~No Ammo";
		}
	}
	else if(AccountData[playerid][pGunType][g_aWeaponSlots[weaponid]] == 0) {
		if(PlayerGuns[playerid][slot][pGunType]) {
			format(typeammo, sizeof(typeammo), "Ammo  Normal");
			//typeammo = "%d", PlayerGuns[playerid][slot][pGunType];
		}
		else {
			typeammo = "~r~No Ammo";
		}
...
GetAmmoStatus(playerid, weaponid) {
	new typeammo[52], slot = g_aWeaponSlots[weaponid];
	if(AccountData[playerid][pGunType][slot] == 1) {
		format(typeammo, sizeof(typeammo), "Surplus");
		//typeammo = "%d~n~SURPLUS", PlayerGuns[playerid][slot][pGunType];
	}
	else {
		typeammo = "~r~Normal";
