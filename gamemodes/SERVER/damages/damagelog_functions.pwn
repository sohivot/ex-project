...
hook OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid, bodypart)
{
	...
	new query[255];
	mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `damagelogs` (`IDs`, `dWeapon`, `dBodyPart`, `dAmount`, `dIssuer`, `dTime`) VALUES ('%d', '%d', '%d', '%.1f', '%s', CURRENT_TIMESTAMP())", AccountData[playerid][pID], weaponid, bodypart, Float:amount, issuer);
...
hook OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid, bodypart)
{
	...
	new query[300];
	mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `damagelogs` SET `IDs`=%d, `dWeapon`=%d, `dBodyPart`=%d, `dAmount`='%.1f', `dIssuer`='%s', `dTime`=CURRENT_TIMESTAMP()", AccountData[playerid][pID], weaponid, bodypart, Float:amount, issuer);
	mysql_tquery(mdb_query, query);
	return 1;
}
...
Damage_Show(playerid)
{
	...
	mysql_format(mdb_query, dbstr, sizeof(dbstr), "SELECT * FROM `damagelogs` WHERE `IDs`=%d ORDER BY `dTime` ASC", AccountData[playerid][pID]);
	execute = mysql_query(mdb_query, dbstr, true);
	new rows = cache_num_rows();
	if(rows)
	{
		if(rows >= 50)
		{
			mysql_tquery(mdb_query, sprintf("DELETE FROM `damagelogs` WHERE `IDs`=%d", AccountData[playerid][pID]));
		}

		new list[4096], date[64], issuer[64], weaponid, Float:damage, bodypart;
		
		format(list, sizeof(list), "Tanggal\tName\tSenjata\tDamage\n");
		for(new x; x < rows; ++x)
		{
		...
		new title[108];
		format(title, sizeof(title), "Damage Logs - "YELLOW"%s", ReturnName(playerid));
...
{
	new dbstr[255], Cache:execute;
	mysql_format(mdb_query, dbstr, sizeof(dbstr), "SELECT * FROM `damagelogs` WHERE `IDs`=%d ORDER BY `dTime` ASC", AccountData[player][pID]);
	execute = mysql_query(mdb_query, dbstr, true);
	new rows = cache_num_rows();
	if(rows)
	{
		if(rows >= 50)
		{
			mysql_tquery(mdb_query, sprintf("DELETE FROM `damagelogs` WHERE `IDs`=%d", AccountData[player][pID]));
		}
...
public DamageShowOffline(playerid, const NamePlayer[])
{
	...
	else 
	{
		...
		mysql_query(mdb_query, sprintf("SELECT * FROM `damagelogs` WHERE `IDs`=%d", playerdbID));
		new rows = cache_num_rows();
		if(rows)
		{
...
CMD:dlogoff(playerid, params[])
{
	...
	new query[512];
	mysql_format(mdb_query, query, sizeof(query), "SELECT `pID` FROM `player_characters` WHERE `Char_Name`='%s'", playerName);
	mysql_tquery(mdb_query, query, "DamageShowOffline", "ds", playerid, playerName);
	return 1;
}
