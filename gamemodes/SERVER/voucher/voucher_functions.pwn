...
CMD:addkompensasi(playerid, params[])
{
    ...
    mysql_tquery(mdb_query, "UPDATE `player_characters` SET `Char_Kompensasi` = 0");
    
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "INSERT INTO `voucher` SET `VoucherID`=%d, `VoucherFee`=%d, `VoucherTime`=%d", VoucherData[0][voucherID], VoucherData[0][voucherFee], VoucherData[0][voucherTime]);
...
CMD:removekompensasi(playerid, params[])
{
    ...
    mysql_tquery(mdb_query, "DELETE FROM `voucher` WHERE `VoucherID`=0");
    return 1;
}
