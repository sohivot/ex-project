...
Tags_Create(playerid)
{
	...
	if((index = Iter_Free(Tags)) != cellmin)
	{
		...
		new output[700];
		mysql_format(mdb_query, output, sizeof(output), "INSERT INTO `tags`(`tagText`, `tagFont`, `tagCreated`, `tagColor`, `tagFontsize`, `tagBold`, `tagOwner`, `tagPosx`, `tagPosy`, `tagPosz`, `tagRotx`, `tagRoty`, `tagRotz`, `tagExpired`) VALUES ('%s','%s','%s','%d','%d','%d','%d','%.3f','%.3f','%.3f','%.3f','%.3f','%.3f','%d')", 
		...
		mysql_tquery(mdb_query, output, "OnTagsCreated", "d", index);
		return index;
	}
	return -1;
}
...
Tags_Delete(index)
{
	...
	{
		mysql_tquery(mdb_query, sprintf("DELETE FROM `tags` WHERE `tagId`='%d';", TagsData[index][tagID]));
