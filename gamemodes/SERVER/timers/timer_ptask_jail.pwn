    ...
    if(AccountData[playerid][pArrest])
    {
        if(AccountData[playerid][pArrestTime] != 0)
        {
            ...
            if(!AccountData[playerid][pArrestTime])
            {
                ...
                new query[255];
                mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Jail`=0, `Char_JailTime`=0 WHERE `pID`=%d", AccountData[playerid][pID]);
    ...
    if(AccountData[playerid][pJail])
    {
        if(AccountData[playerid][pJailTime] != 0)
        {
            ...
            if(!AccountData[playerid][pJailTime])
            {
                ...
                new cQuery[178];
                mysql_format(mdb_query, cQuery, sizeof(cQuery), "DELETE FROM `warninglogs` WHERE `WarnType` = 2 AND `pID` = '%d'", AccountData[playerid][pID]);
