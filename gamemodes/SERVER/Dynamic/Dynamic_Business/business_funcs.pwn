...
CMD:createbisnis(playerid, params[])
{
	...
	mysql_format(mdb_query, query, sizeof(query), "INSERT INTO bisnis SET ID='%d', owner='%s', price='%d', type='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f', name='%s'", bid, BisnisData[bid][bOwner], BisnisData[bid][bPrice], BisnisData[bid][bType], BisnisData[bid][bExtposX], BisnisData[bid][bExtposY], BisnisData[bid][bExtposZ], BisnisData[bid][bExtposA], BisnisData[bid][bName]);
	mysql_tquery(mdb_query, query, "OnBisnisCreated", "i", bid);
	return 1;
}
...
CMD:editbisnis(playerid, params[])
{
	...
	else if(!strcmp(type, "delete", true))
    {
		...
		new query[128];
		mysql_format(mdb_query, query, sizeof(query), "DELETE FROM bisnis WHERE ID=%d", bid);
