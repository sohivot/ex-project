...
{
    new query[596];
    mysql_format(mdb_query, query, sizeof(query), "UPDATE `trash` SET `Model`=%d, `Cooldown`=%d, `PosX`=%f, `PosY`=%f, `PosZ`=%f, `RotX`=%f, `RotY`=%f, `RotZ`=%f, `Interior`=%d, `World`=%d WHERE `ID`=%d",
...
CMD:addtrash(playerid, params[])
{
    ...
    new query[598];
    mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `trash` SET `ID`=%d, `Model`=%d, `Cooldown`=%d, `PosX`=%f, `PosY`=%f, `PosZ`=%f, `RotX`=%f, `RotY`=%f, `RotZ`=%f,\
    ...
    mysql_tquery(mdb_query, query, "OnTrashCreated", "d", id);
    return 1;
}
...
CMD:edittrash(playerid, params[])
{
    ...
    else if(!strcmp(option, "delete", true))
    {
        ...
        new query[125];
        mysql_format(mdb_query, query, sizeof(query), "DELETE FROM `trash` WHERE `ID`=%d", id);
        mysql_tquery(mdb_query, query); 
    }
    return 1;
}
...
task TrashCooldown_Update[1000]()
{
    foreach(new id : Trash)
    {
        ...
        new query[255];
        mysql_format(mdb_query, query, sizeof(query), "UPDATE `trash` SET `Cooldown`=%d WHERE `ID`=%d", TrashData[id][trashCooldown], id);
        mysql_tquery(mdb_query, query);
    }
    return 1;
}