...
{
    new query[1218];
    mysql_format(mdb_query, query, sizeof(query), "UPDATE `buttons` SET `DoorModel`=%d, `Faction`=%d, `Faction2`=%d, `Family`=%d, `Owner`=%d, `Speed`=%f, `ButtonX`=%f, `ButtonY`=%f, `ButtonZ`=%f, `ButtonRX`=%f, `ButtonRY`=%f, `ButtonRZ`=%f,\
...
CMD:addbutton(playerid, params[])
{
    ...
    new query[596];
    mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `buttons` SET `ID`=%d, `ButtonX`=%f, `ButtonY`=%f, `ButtonZ`=%f, `ButtonRX`=%f, `ButtonRY`=%f, `ButtonRZ`=%f, `Faction`=0, `Faction2`=0, `Family`=-1, `World`=%d, `Interior`=%d",
    ...
    mysql_tquery(mdb_query, query, "OnButtonCreated", "dd", playerid, id);
    return 1;
}
...
CMD:removebutton(playerid, params[])
{
    ...
    new query[255];
    mysql_format(mdb_query, query, sizeof(query), "DELETE FROM `buttons` WHERE `ID`=%d", id);
    mysql_tquery(mdb_query, query);
