...
CMD:adddeer(playerid, params[])
{
    ...
    mysql_format(mdb_query, twsmk, sizeof(twsmk), "INSERT INTO `hunting` SET `DeerID`=%d, `DeerX`=%f, `DeerY`=%f, `DeerZ`=%f, `DeerRX`=%f, `DeerRY`=%f, `DeerRZ`=%f, \
    ...
    mysql_tquery(mdb_query, twsmk, "OnDeerCreated", "dd", playerid, id);
    return 1;
}
...
CMD:removedeer(playerid, params[])
{
    ...
    mysql_format(mdb_query, icsr, sizeof(icsr), "DELETE FROM `hunting` WHERE `DeerID`=%d", id);
...
task HuntingUpdate[1000]()
{
    ...
    foreach(new i : Hunt)
    {
        ...
        new icsr[255];
        mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `hunting` SET `DeerTime`=%d WHERE `DeerID`=%d", HuntData[i][DeerTime], i);
        mysql_tquery(mdb_query, icsr);
    }
    return 1;
}
