...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        {
            Dialog_Show(playerid, RUSUN_BRANKAS, DIALOG_STYLE_LIST, ""Imajinasi"Imajinasi Roleplay "WHITE"- %s",
            "Undang\
            \n"GRAY"Pakaian\
            \nMembuang Pakaian\
        ...
        if ((rsid = Rusun_Nearest(playerid)) != -1)
        {
            if (RusunsData[rsid][rOwnerID] != 0)
            {
                ...
                else
                {
                    ...
                    Dialog_Show(playerid, RUSUN_OWNED, DIALOG_STYLE_LIST, sprintf(""Imajinasi"Imajinasi Roleplay "WHITE"- %s", RusunsData[rsid][rName]),
                    "Masuk\
            ...
            {
                AccountData[playerid][pInRusun] = rsid;
                Dialog_Show(playerid, RUSUN_MENU, DIALOG_STYLE_TABLIST, sprintf(""Imajinasi"Imajinasi Roleplay "WHITE"- %s", RusunsData[rsid][rName]),
...
Dialog:RUSUN_BRANKAS(playerid, response, listitem, inputtext[])
{
    ...
    switch(listitem)
    {
        case 0:
        {
            ...
            Dialog_Show(playerid, RUSUN_INVITE, DIALOG_STYLE_LIST, ""Imajinasi"Imajinasi Roleplay "WHITE"- Undang Teman", frmxt, "Pilih", "Batal");
        }
        case 1:
        {
        ...
        case 3:
        {
            Dialog_Show(playerid, RUSUN_BROPTION, DIALOG_STYLE_LIST, ""Imajinasi"Imajinasi Roleplay "WHITE"- Brankas",
...
Dialog:RUSUN_INVITE(playerid, response, listitem, inputtext[])
{
    ...
    if ((id = RusunInside(playerid)) != -1)
    {
        ...
        Dialog_Show(targetid, RUSUN_INVITECONF, DIALOG_STYLE_MSGBOX, ""Imajinasi"Imajinasi Roleplay "WHITE"- Undang Teman",
    ...
    switch(listitem)
    {
        case 0: //deposit
        {
            ...
            if(cache_num_rows() > 0)
            {
                ...
                Dialog_Show(playerid, RUSUNVAULT_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, ""Imajinasi"Imajinasi Roleplay "WHITE"- Brankas", str, "Pilih", "Batal");
            }
            else 
            {
                PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
        ...
        case 1: //withdraw
        {
            ...
            format(str, sizeof(str), "Nama Item\tJumlah\tBerat (%.3f/80kg)\n", AccountData[playerid][pRusunCapacity]);
            mysql_format(mdb_query, icsr, sizeof(icsr), "SELECT * FROM `player_rusunstorage` WHERE `ID`=%d", AccountData[playerid][pID]);
            ...
            if(cache_num_rows() > 0)
            {
                ...
                Dialog_Show(playerid, RUSUNVAULT_WITHDRAW, DIALOG_STYLE_TABLIST_HEADERS, ""Imajinasi"Imajinasi Roleplay "WHITE"- Brankas", str, "Pilih", "Batal");
            }
            else 
            {
                return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""Imajinasi"Imajinasi Roleplay "WHITE"- Brankas",
...
Dialog:RUSUNVAULT_DEPOSIT(playerid, response, listitem, inputtext[])
{
    ...
    if(cache_num_rows() > 0)
    {
        if(listitem >= 0 && listitem < cache_num_rows())
        {
            ...
            Dialog_Show(playerid, RUSUNVAULT_IN, DIALOG_STYLE_INPUT, ""Imajinasi"Imajinasi Roleplay "WHITE"- Brankas",
            shstr, "Input", "Batal");
        }
    }
    return 1;
...
Dialog:RUSUNVAULT_IN(playerid, response, listitem, inputtext[])
{
    ...
    new invstr[1218];
    mysql_format(mdb_query, shstr, sizeof(shstr), "SELECT * FROM `player_rusunstorage` WHERE `ID`=%d AND `rsItemName`='%s'", AccountData[playerid][pID], RusunBrankas[playerid][rusunbrankasTemp]);
    mysql_query(mdb_query, shstr);
    if(cache_num_rows() > 0)
    {
        mysql_format(mdb_query, invstr, sizeof(invstr), "UPDATE `player_rusunstorage` SET `rsItemQuantity` = `rsItemQuantity` + %d WHERE `ID`=%d AND `rsItemName`='%s'", quantity, AccountData[playerid][pID], RusunBrankas[playerid][rusunbrankasTemp]);
        mysql_tquery(mdb_query, invstr, "OnRusunDepo", "d", playerid);
    }
    else
    {
        mysql_format(mdb_query, invstr, sizeof(invstr), "INSERT INTO `player_rusunstorage` SET `ID`=%d, `rsItemName`='%s', `rsItemModel`=%d, `rsItemQuantity`=%d", AccountData[playerid][pID], RusunBrankas[playerid][rusunbrankasTemp], RusunBrankas[playerid][rusunbrankasModel], quantity);
        mysql_tquery(mdb_query, invstr, "OnRusunDepo", "d", playerid);
    }
    return 1;
}
...
Dialog:RUSUNVAULT_WITHDRAW(playerid, response, listitem, inputtext[])
{
    ...
    new tss[1218];
    mysql_format(mdb_query, tss, sizeof(tss), "SELECT * FROM `player_rusunstorage` WHERE `ID`=%d", AccountData[playerid][pID]);
    mysql_query(mdb_query, tss);
    if(cache_num_rows() > 0)
    {
        if(listitem >= 0 && listitem < cache_num_rows())
        {
            ...
            Dialog_Show(playerid, RUSUNVAULT_OUT, DIALOG_STYLE_INPUT, ""Imajinasi"Imajinasi Roleplay "WHITE"- Brankas",
            shstr, "Input", "Batal");
        }
    }
    return 1;
...
Dialog:RUSUNVAULT_OUT(playerid, response, listitem, inputtext[])
{
    ...
    {
        mysql_format(mdb_query, tss, sizeof(tss), "UPDATE `player_rusunstorage` SET `rsItemQuantity`=%d WHERE `rsItemID`=%d", RusunBrankas[playerid][rusunbrankasQuant], RusunBrankas[playerid][rusunbrankasID]);
        mysql_tquery(mdb_query, tss);
    } 
    else 
    {
        mysql_format(mdb_query, tss, sizeof(tss), "DELETE FROM `player_rusunstorage` WHERE `rsItemID`=%d", RusunBrankas[playerid][rusunbrankasID]);
        mysql_tquery(mdb_query, tss);
    }
