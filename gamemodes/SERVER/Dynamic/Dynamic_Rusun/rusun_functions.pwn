...
{
    new query[1218];
    mysql_format(mdb_query, query, sizeof(query), "UPDATE `dynamic_rusun` SET `RusunOwner`='%s', `RusunOwnerID`=%d, `RusunPrice`=%d, `RusunName`='%s', `RusunInterior`=%d, `RusunExtInterior`=%d, `RusunWorld`=%d,\
...
CMD:addrusun(playerid, params[])
{
    ...
    new query[596];
    mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `dynamic_rusun` SET `rID`=%d, `RusunOwner`='%s', `RusunOwnerID`=%d, `RusunPrice`=%d, `RusunName`='%s', `ExtPosX`=%f, `ExtPosY`=%f, `ExtPosZ`=%f, `ExtPosA`=%f,\
    ...
    mysql_tquery(mdb_query, query, "OnRusunCreated", "dd", playerid, id);
    return 1;
}
...
CMD:editrusun(playerid, params[])
{
    ...
    else if(!strcmp(option, "reset", true))
    {
        ...
        mysql_tquery(mdb_query, sprintf("UPDATE `player_characters` SET `Char_HasRusunID`=-1 WHERE `pID`=%d", RusunData[id][rusunOwnerID]));
        Rusun_Reset(id);
        Rusun_Refresh(id);
        Rusun_Save(id);
    ...
    else if(!strcmp(option, "delete", true))
    {
        ...
        new query[178];
        mysql_format(mdb_query, query, sizeof(query), "DELETE FROM `dynamic_rusun` WHERE `rID`=%d", id);
        mysql_tquery(mdb_query, query);
    }
    return 1;
}
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    else if(dialogid == DIALOG_RUSUNVAULT_IN)
    {
        ...
        if(cache_num_rows() > 0)
        {
            ...
            mysql_tquery(mdb_query, invstr, "OnRusunDeposit", "d", playerid);
        }
        else
        {
...
AddCapacityRusun(playerid, const item[], quantity)
{
    ...
    new sctr[596];
    mysql_format(mdb_query, sctr, sizeof(sctr), "UPDATE `player_characters` SET `Char_RusunStorage`=%f WHERE `pID`=%d", AccountData[playerid][pRusunCapacity], AccountData[playerid][pID]);
    mysql_tquery(mdb_query, sctr);
    return 1;
}
