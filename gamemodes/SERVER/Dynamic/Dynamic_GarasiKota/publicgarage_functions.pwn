...
CMD:addgarkot(playerid, params[])
{
    ...
    new cQuery[255];
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "INSERT INTO `public_garage` SET `pgID`=%d, `pgName`='%s', `pgPosX`=%f, `pgPosY`=%f, `pgPosZ`=%f, `pgSpawnX`=%f, `pgSpawnY`=%f, `pgSpawnZ`=%f, `pgInterior`=%d, `pgWorld`=%d",
    ...
    mysql_tquery(mdb_query, cQuery, "OnPublicGarageAdded", "dd", playerid, id);
    return 1;
}
...
CMD:editgarkot(playerid, params[])
{
    ...
    else if(!strcmp(type, "delete", true))
    {
        ...
        new query[125];
        mysql_format(mdb_query, query, sizeof(query), "DELETE FROM `public_garage` WHERE `pgID`=%d", did);
...
hook OnPlayerEnterDynArea(playerid, STREAMER_TAG_AREA:areaid)
{
    foreach(new i : PublicGarage)
    {
        if(areaid == PublicGarage[i][pgArea])
        {
            ...
            {
                ShowKey(playerid, "[ALT] Akses Garasi Umum");
            }
