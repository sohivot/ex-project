...
CMD:addatm(playerid, params[])
{
	...
	mysql_format(mdb_query, query, sizeof(query), "INSERT INTO atms SET id='%d', posx='%f', posy='%f', posz='%f', posrx='%f', posry='%f', posrz='%f', interior='%d', world='%d'", id, AtmData[id][atmX], AtmData[id][atmY], AtmData[id][atmZ], AtmData[id][atmRX], AtmData[id][atmRY], AtmData[id][atmRZ], GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid));
	mysql_tquery(mdb_query, query, "OnAtmCreated", "ii", playerid, id);
	return 1;
}
...
CMD:removeatm(playerid, params[])
{
	...
	mysql_format(mdb_query, query, sizeof(query), "DELETE FROM atms WHERE id=%d", id);
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		case DIALOG_ATM_TRANSFER:
		{
			...
			mysql_tquery(mdb_query, query, "SearchRekening", "dd", playerid, strval(inputtext));
			return 1;	
		}
		...
		case DIALOG_ATM_DEPOSIT:
		{
			...
			new query[200];
			mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_BankMoney`=%d, `Char_Money`=%d WHERE `pID`=%d", AccountData[playerid][pBankMoney], AccountData[playerid][pMoney], AccountData[playerid][pID]);
			mysql_tquery(mdb_query, query);
		}
...
hook OnPlayerEnterDynArea(playerid, areaid)
{
	foreach(new i : ATMS)
	{
		if(areaid == AtmData[i][atmArea])
		{
			...
			{
				ShowKey(playerid, "[ALT]- Akses ATM");
			}
		}
	}
	return 1;
