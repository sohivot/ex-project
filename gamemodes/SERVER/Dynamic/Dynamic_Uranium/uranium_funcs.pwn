...
{
    new cQuery[522];
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "UPDATE `uranium` SET `UraniumX`='%f', `UraniumY`='%f', `UraniumZ`='%f', `UraniumRX`='%f', `UraniumRY`='%f', `UraniumRZ`='%f', `Interior`=%d, `World`=%d, `Timer`=%d WHERE `ID`=%d",
...
CMD:adduranium(playerid, params[])
{
    ...
    new query[298];
    mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `uranium` (`ID`, `UraniumX`, `UraniumY`, `UraniumZ`, `UraniumRX`, `UraniumRY`, `UraniumRZ`, `Interior`, `World`, `Timer`) VALUES ('%d', '%f', '%f', '%f', '%f', '%f', '%f', '%d', '%d', '%d')",
    ...
    mysql_tquery(mdb_query, query, "OnUraniumCreated", "ii", playerid, id);
    return 1;
}
...
CMD:edituranium(playerid, params[])
{
    ...
    if(!strcmp(option, "delete", true))
    {
        ...
        mysql_tquery(mdb_query, sprintf("DELETE FROM `uranium` WHERE `ID`=%d", id));
...
hook OnPlayerEnterDynArea(playerid, STREAMER_TAG_AREA:areaid)
{
    for(new i = 0; i < MAX_DYNAMIC_URANIUM; i ++) if (UraniumData[i][UraniumPos][0] != 0.0)
    {
        ...
        {
            ShowKey(playerid, "[ALT] - Take Uranium");
        }
    }
    return 1;
}
