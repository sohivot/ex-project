...
CMD:addgudang(playerid, params[])
{
    ...
    new tss[596];
    mysql_format(mdb_query, tss, sizeof(tss), "INSERT INTO `gudang` SET `GudangID`=%d, `GudangName`='%s', `GudangPrice`=%d, `GudangX`=%f, `GudangY`=%f, `GudangZ`=%f, `GudangInterior`=%d, `GudangWorld`=%d",
    ...
    mysql_tquery(mdb_query, tss, "OnGudangCreated", "dd", playerid, id);
    return 1;
}
...
CMD:removegudang(playerid, params[])
{
	...
	mysql_format(mdb_query, icsr, sizeof(icsr), "DELETE FROM `gudang` WHERE `GudangID` = %d", id);
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_GUDANG_BUY)
    {
        ...
        switch(listitem)
        {
            case 0: //sewa
            {
                ...
                new icsr[255];
                mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_HasGudangID`=%d WHERE `pID`=%d", AccountData[playerid][pHasGudangID], AccountData[playerid][pID]);
                mysql_tquery(mdb_query, icsr);
    ...
    else if(dialogid == DIALOG_GUDANGVAULT)
    {
        ...
        switch(listitem)
        {
            ...
            case 1: //withdraw
            {
                ...
                mysql_format(mdb_query, jss, sizeof(jss), "SELECT * FROM `player_gudang` WHERE `ID`=%d", AccountData[playerid][pID]);
                mysql_query(mdb_query, jss);
                if(cache_num_rows() > 0)
                {
    ...
    else if(dialogid == DIALOG_GUDANGVAULT_IN)
    {
        ...
        new invstr[1028];
        mysql_format(mdb_query, shstr, sizeof(shstr), "SELECT * FROM `player_gudang` WHERE `ID`=%d AND `itemName`='%s'", AccountData[playerid][pID], GudangBrankas[playerid][gudangBrankasTemp]);
        mysql_query(mdb_query, shstr);
        if(cache_num_rows() > 0)
        {
            mysql_format(mdb_query, invstr, sizeof(invstr), "UPDATE `player_gudang` SET `itemQuantity` = `itemQuantity` + %d WHERE `ID`=%d AND `itemName`='%s'", quantity, AccountData[playerid][pID], GudangBrankas[playerid][gudangBrankasTemp]);
            mysql_tquery(mdb_query, invstr, "OnGudangDeposit", "d", playerid);
        }
        else 
        {
            mysql_format(mdb_query, invstr, sizeof(invstr), "INSERT INTO `player_gudang` SET `ID`=%d, `itemName`='%s', `itemModel`=%d, `itemQuantity`=%d", AccountData[playerid][pID], GudangBrankas[playerid][gudangBrankasTemp], GudangBrankas[playerid][gudangBrankasModel], quantity);
            mysql_tquery(mdb_query, invstr, "OnGudangDeposit", "d", playerid);
        }
    }
    else if(dialogid == DIALOG_GUDANGVAULT_WITHDRAW)
    {
        ...
        mysql_format(mdb_query, tss, sizeof(tss), "SELECT * FROM `player_gudang` WHERE `ID`=%d", AccountData[playerid][pID]);
        mysql_query(mdb_query, tss);
        if(cache_num_rows() > 0)
        {
    ...
    else if(dialogid == DIALOG_GUDANGVAULT_OUT)
    {
        ...
        {
            mysql_format(mdb_query, jts, sizeof(jts), "UPDATE `player_gudang` SET `itemQuantity`=%d WHERE `itemID`=%d", GudangBrankas[playerid][gudangBrankasQuant], GudangBrankas[playerid][gudangBrankasID]);
            mysql_tquery(mdb_query, jts);
        }
        else
        {
            mysql_format(mdb_query, jts, sizeof(jts), "DELETE FROM `player_gudang` WHERE `itemID`=%d", GudangBrankas[playerid][gudangBrankasID]);
            mysql_tquery(mdb_query, jts);
        }
...
Dialog:GUDANG_STOPCONF(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        new tss[255];
        mysql_format(mdb_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_HasGudangID`=-1, `Char_GudangRentTime`=0 WHERE `pID`=%d", AccountData[playerid][pID]);
...
AddCapacityGudang(playerid, const item[], quantity)
{
    ...
    new sctr[596];
    mysql_format(mdb_query, sctr, sizeof(sctr), "UPDATE `player_characters` SET `Char_GudangStorage`=%f WHERE `pID`=%d", AccountData[playerid][pGudangCapacity], AccountData[playerid][pID]);
