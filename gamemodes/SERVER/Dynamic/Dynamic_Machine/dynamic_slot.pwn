...
{
    new query[596];
    mysql_format(mdb_query, query, sizeof(query), "UPDATE `slotmachine` SET `PosX`=%f, `PosY`=%f, `PosZ`=%f, `RotX`=%f, `RotY`=%f, `RotZ`=%f, `Interior`=%d, `World`=%d WHERE `ID`=%d",
...
CMD:addslot(playerid, params[])
{
    ...
    new query[255];
    mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `slotmachine` SET `ID`=%d, `PosX`=%f, `PosY`=%f, `PosZ`=%f, `RotX`=%f, `RotY`=%f, `RotZ`=%f, `Interior`=%d, `World`=%d",
    ...
    mysql_tquery(mdb_query, query, "OnSlotCreated", "dd", playerid, id);
    return 1;
}
...
CMD:removeslot(playerid, params[])
{
    ...
    new query[200];
    mysql_format(mdb_query, query, sizeof(query), "DELETE FROM `slotmachine` WHERE `ID`=%d", id);
    mysql_tquery(mdb_query, query);
    return 1;
}
