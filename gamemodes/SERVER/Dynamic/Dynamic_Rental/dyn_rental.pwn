...
RentalRefresh(rentid)
{
    if(rentid != -1)
    {
        ...
        RentalData[rentid][RentalLabel] = CreateDynamic3DTextLabel(sprintf(""LIGHTGREY"[%s]\n"BLUEJEGE"Rental Vehicle\n"GREEN"[ALT]"WHITE" untuk mengakses rental vehicle", string), COLOR_WHITE, RentalData[rentid][RentalPos][0], RentalData[rentid][RentalPos][1], RentalData[rentid][RentalPos][2] + 0.55, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1);
    }
    return 1;
}
...
CMD:editrent(playerid, params[])
{
    ...
    else if(!strcmp(type, "delete", true))
    {
        ...
        mysql_tquery(mdb_query, sprintf("DELETE FROM bike_rentals WHERE RentID='%d'", rentid));
    ...
    switch(RentalData[id][RentalType])
    {
        case 1: // Motor
        {
            switch(listitem)
            {
                ...
                case 1:
                {
                    if(AccountData[playerid][pMoney] < 200) return ShowTDN(playerid, NOTIFICATION_ERROR, "Uang anda tidak mencukupi!");
                    new count = 0;
                    foreach(new i : PvtVehicles)
                    {
                        if(PlayerVehicle[i][pVehExists] && PlayerVehicle[i][pVehOwnerID] == AccountData[playerid][pID])
                    ...
                    foreach(new i : PvtVehicles) if(PlayerVehicle[i][pVehExists]) if(IsValidVehicle(PlayerVehicle[i][pVehPhysic]) && !IsABoat(PlayerVehicle[i][pVehPhysic]))
                    {
                        if(PlayerVehicle[i][pVehOwnerID] == AccountData[playerid][pID])
                        {
                            if(PlayerVehicle[i][pVehRental] != -1 && PlayerVehicle[i][pVehRentTime] != 0)
                            {
                                ...
                                mysql_tquery(mdb_query, sprintf("DELETE FROM player_vehicles WHERE id = '%d'", PlayerVehicle[i][pVehID]));
...
VehicleRental_Create(playerid, modelid, rentaltime, rentalid, Float:x, Float:y, Float:z, Float:angle, color1, color2, price, worldid, interiorid)
{
    ...
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "INSERT INTO `player_vehicles` (`PVeh_OwnerID`, `PVeh_ModelID`, `PVeh_Parked`, `PVeh_Housed`, `PVeh_RentTime`, `PVeh_Rental`, `PVeh_Plate`, `PVeh_Health`, `PVeh_Fuel`, `PVeh_PosX`, `PVeh_PosY`, `PVeh_PosZ`, `PVeh_PosA`, \
    ...
    mysql_tquery(mdb_query, cQuery, "OnVehRentalCreated", "ii", playerid, i);
    return 1;
}

ptask VehRent_Update[999](playerid)
{
