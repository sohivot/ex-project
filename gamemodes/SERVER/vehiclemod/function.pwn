...
Vehicle_ObjectAddObjects(playerid, vehicleid, model, type)
{
    ...
    { 
        if(VehicleObjects[vehicleid][slot][vehObjectExists] == false)
        {
            ...
            mysql_tquery(mdb_query, query_string, "Vehicle_ObjectDB", "dd", vehicleid, slot);
            return 1;
        }
    }
	return 0;
...
Vehicle_ObjectDelete(vehicleid, slot)
{
    ...
    mysql_tquery(mdb_query, query_string);
    return 1;
}
...
ResetEditing(playerid)
{
    switch(AccountData[playerid][pEditingMode])
    {
        case ROADBLOCK:
        {
            if(AccountData[playerid][pEditRoadblock] != -1)
            {
                Barricade_Sync(AccountData[playerid][pEditRoadblock]);
                AccountData[playerid][pEditRoadblock] = -1;
            }
        }
    }
    AccountData[playerid][pEditingMode] = NOTHING;
