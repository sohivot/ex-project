...
CMD:sc(playerid, params[])
{
    ...
    new cQuery[2118];
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "INSERT INTO `clothes_wardrobe` SET `IDs`=%d, `clothesName`='%s', `clothesModel`=%d,\
    ...
    mysql_tquery(mdb_query, cQuery, "OnClothesDeposit", "d", playerid);
    return 1;
}
...
{
    new dbstr[255];
    mysql_format(mdb_query, dbstr, sizeof(dbstr), "SELECT * FROM `clothes_wardrobe` WHERE `IDs`=%d", AccountData[playerid][pID]);
    mysql_query(mdb_query, dbstr);
    ...
    switch(dialogid)
    {
        ...
        case DIALOG_CLOTHES_DELETE:
        {
            ...
            if(cache_num_rows())
            {
                ...
                new query[200];
                mysql_format(mdb_query, query, sizeof(query), "DELETE FROM `clothes_wardrobe` WHERE `clothesModel` = %d", clothesmodel);
