    ...
    if(response)
    {
        switch(listitem)
        {
            case 0:
            {
                ...
                new Cache:checkname = mysql_query(mdb_query, sprintf("SELECT `PartnerName` FROM `users` WHERE `pID` = '%d' LIMIT 1;",AccountData[playerid][pID]));
                if(cache_num_rows())
                {
                ...
                new Cache:checklove = mysql_query(mdb_query, sprintf("SELECT `Love` FROM `users` WHERE `pID` = '%d' LIMIT 1;", AccountData[playerid][pPartnerID]));
                if(cache_num_rows())
                {
            ...
            case 1:
            {
                ...
                if(IsPlayerConnected(targetid))
                {
                    ...
                    { 
                        ...
                        mysql_tquery(mdb_query, sprintf("UPDATE `users` SET `PartnerGift` = '%d' WHERE pID = %d;", gift_date, AccountData[playerid][pID]));
                        mysql_tquery(mdb_query, sprintf("UPDATE `users` SET `PartnerGift` = '%d' WHERE pID = %d;", gift_date, AccountData[targetid][pID]));
                ...
                {
                    new partnerlove;
                    new Cache:checkaccount = mysql_query(mdb_query, sprintf("SELECT `Love` FROM `users` WHERE `pID` = '%d' LIMIT 1;",AccountData[playerid][pPartnerID]));
                    if(cache_num_rows())
                    {
                    ...
                    mysql_tquery(mdb_query, sprintf("UPDATE `users` SET `PartnerGift` = '%d' WHERE pID = %d;", gift_date, AccountData[playerid][pPartnerID]));
                    //Ngeupdate love si partnernya kalau offline
                    mysql_tquery(mdb_query, sprintf("UPDATE `users` SET `Love` = '%d' WHERE pID = %d;", partnerlove+10, AccountData[playerid][pPartnerID]));
                }                
            }
            case 2:
            {
            ...
            {   
                ...
                {
                    mysql_tquery(mdb_query, sprintf("UPDATE `users` SET `Status` = '0', `Love` = '0.0', `PartnerID` = '0', `PartnerName` = 'Single' WHERE pID = %d", AccountData[playerid][pPartnerID]));
...
CMD:relationcheck(playerid, params[])
{
    ...
    new Cache:checkname = mysql_query(mdb_query, sprintf("SELECT `PartnerName` FROM `users` WHERE `pID` = '%d' LIMIT 1;",AccountData[userid][pID]));
    if(cache_num_rows())
    {
        cache_get_value_name(0, "PartnerName", partnername, 32);
        cache_delete(checkname);
    ...
    new Cache:checklove = mysql_query(mdb_query, sprintf("SELECT `Love` FROM `users` WHERE `pID` = '%d' LIMIT 1;", AccountData[userid][pPartnerID]));
    if(cache_num_rows())
    {
        cache_get_value_name_float(0, "Love", partnerlove);
        cache_delete(checklove);
