...
{
	new query[256];
	mysql_format(mdb_query, query, sizeof(query), "SELECT `Char_Name` FROM `player_characters` WHERE `Char_UCP`= '%s' LIMIT %d;", AccountData[playerid][pUCP], MAX_CHARS + AccountData[playerid][pExtraChar]);
	mysql_tquery(mdb_query, query, "LoadCharacter", "d", playerid);
	return 1;
}
...
/*
stock serverUiLogin(playerid, bool:status)
{
	if(status) {
		forex(i, 32) {
			TextDrawShowForPlayer(playerid, serverLogin[i]);
		}
	} else {
		forex(i, 32) {
			TextDrawHideForPlayer(playerid, serverLogin[i]);
		}
	}
	return 1;
}*/
...
public CheckPlayerUCP(playerid, rcc)
{
	...
	for(new x = 0; x < 7; x ++)
	{
		TextDrawShowForPlayer(playerid, exeren[x]);
	}
...
public CheckBanUCP(playerid)
{
	if(cache_num_rows())
	{
		...
		{
			new query[248];
			mysql_format(mdb_query, query, sizeof(query), "DELETE FROM player_bans WHERE name = '%s'", AccountData[playerid][pUCP]);
		...
		else
		{
			new query[248], PlayerIP[16], strgbg[712];
  			mysql_format(mdb_query, query, sizeof query, "UPDATE `player_bans` SET `last_activity_timestamp` = '%d' WHERE `name` = '%s'", gettime(), AccountData[playerid][pUCP]);
			mysql_tquery(mdb_query, query);
...
{	
	...
	cache_get_value_name_int(0, "Char_Story", AccountData[playerid][pCharStory]);
	cache_get_value_name_int(0, "Char_AdminBS", AccountData[playerid][pAdminBS]);//buatload
	cache_get_value_name_int(0, "married", AccountData[playerid][pMarried]);
	cache_get_value_name(0, "marriedto", AccountData[playerid][pMarriedTo], 50);
	...
	sscanf(str, "p<|>ddddddddddddd", AccountData[playerid][pGunType][0], AccountData[playerid][pGunType][1],
	AccountData[playerid][pGunType][2], AccountData[playerid][pGunType][3], AccountData[playerid][pGunType][4],
	AccountData[playerid][pGunType][5], AccountData[playerid][pGunType][6], AccountData[playerid][pGunType][7],
	AccountData[playerid][pGunType][8], AccountData[playerid][pGunType][9], AccountData[playerid][pGunType][10], 
	AccountData[playerid][pGunType][11], AccountData[playerid][pGunType][12]);
	...
	cache_get_value_name_int(0, "Char_KtpTime", AccountData[playerid][KtpTime]);
	...
	new cQuery[1048];
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "SELECT * FROM `inventory` WHERE `ID`=%d", AccountData[playerid][pID]);
	mysql_tquery(mdb_query, cQuery, "LoadPlayerInventory", "d", playerid);
	
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "SELECT * FROM `weaponsettings` WHERE `Owner`=%d", AccountData[playerid][pID]);
	mysql_tquery(mdb_query, cQuery, "OnWeaponsLoaded", "d", playerid);
	...
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "SELECT * FROM `blacklist_players` WHERE `PID` = %d", AccountData[playerid][pID]);
	mysql_tquery(mdb_query, cQuery, "LoadBlacklistInfo", "d", playerid);

	mysql_format(mdb_query, cQuery, sizeof(cQuery), "SELECT * FROM `toys` WHERE `Owner` = '%s' LIMIT 1", AccountData[playerid][pName]);
	mysql_tquery(mdb_query, cQuery, "LoadPlayerToys", "d", playerid);
	...
	if(gettime() > AccountData[playerid][LastSpawn])
	{
		...
		else
		{
			...
			new rand = random(sizeof(SpawnPelabuhan));
			SetPlayerPos(playerid, SpawnPelabuhan[rand][0], SpawnPelabuhan[rand][1], SpawnPelabuhan[rand][2]);
			SetPlayerInteriorEx(playerid, 0);
			SetPlayerVirtualWorldEx(playerid, 0);
			AccountData[playerid][playerClickSpawn] = 0;
...
public MakeWargaOffline(playerid, name[]) 
{
	...
	else
	{
		...
		new query[155];
		mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Family`=-1, `Char_FamilyRank`=0, `Char_Faction`=0, `Char_FactionRank`=0 WHERE `pID`=%d", RegID);
		mysql_tquery(mdb_query, query);
...
public OnPlayerRegister(playerid)
{
	...
	AccountData[playerid][KtpTime] = gettime() + (7 * 86400); // Seminggu
	...
	mysql_format(mdb_query, query, sizeof(query), "SELECT `Char_BankRek` FROM `player_characters` WHERE `Char_BankRek`=%d", rekening);
	mysql_tquery(mdb_query, query, "BankRekening", "id", playerid, rekening);
	...
	mysql_format(mdb_query, query, sizeof(query), "SELECT Char_PhoneNum FROM player_characters WHERE Char_PhoneNum='%s'", nope);
	mysql_tquery(mdb_query, query, "PhoneNumber", "is", playerid, nope);

	mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `blacklist_players` SET `PID` = %d", AccountData[playerid][pID]);
	mysql_tquery(mdb_query, query);	
...
public BankRekening(playerid, rekening)
{
	if(cache_num_rows())
	{
		...
		mysql_format(mdb_query, query, sizeof(query), "SELECT `Char_BankRek` FROM `player_characters` WHERE `Char_BankRek`=%d", rekening);
		mysql_tquery(mdb_query, query, "BankRekening", "id", playerid, rek);
		SendClientMessageEx(playerid, -1, "[i] Ada rekening yang sama dengan rekeningmu. Kami sedang mencarikan ulang untuk anda...");
	}
	else 
	{
		new query[125];
		mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_BankRek`=%d WHERE `pID`=%d", rekening, AccountData[playerid][pID]);
		mysql_tquery(mdb_query, query);
...
public PhoneNumber(playerid, phone[])
{
	if(cache_num_rows() > 0)
	{
		...
		mysql_format(mdb_query, query, sizeof(query), "SELECT `Char_PhoneNum` FROM `player_characters` WHERE `Char_PhoneNum`='%s'", phones);
		mysql_tquery(mdb_query, query, "PhoneNumber", "is", playerid, phones);
		SendClientMessageEx(playerid, -1, "[i] Sudah ada yang menggunakan nomor telepon ini. kami akan mencarikan yang baru");
	}
	else 
	{
		new query[128];
		mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_PhoneNum`='%s' WHERE `pID`='%d'", phone, AccountData[playerid][pID]);
		mysql_tquery(mdb_query, query);
...
public a_ChangeAdminName(otherplayer, playerid, nname[])
{
	...
	else
	{
		...
		new query[512];
	    mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_AdminName`='%s' WHERE `pID`=%d", nname, AccountData[otherplayer][pID]);
		mysql_tquery(mdb_query, query);
...
public OnOBanQueryData(adminid, UCPToBan[], banReason[], banTime)
{
	...
	else 
	{
		...
		new twsmk[512];
		mysql_format(mdb_query, twsmk, sizeof(twsmk), "INSERT INTO player_bans (name, ip, admin, reason, ban_date, ban_expire) VALUES ('%s', '%s', '%s', '%s', UNIX_TIMESTAMP(), %d)", UCPToBan, PlayerIP, AccountData[adminid][pAdminname], banReason, datez);
		mysql_tquery(mdb_query, twsmk);
	}
	return 1;
}
...
public UsingDokterLokal(playerid)
{
	...
	else
	{
		...
		PlayerTextDrawHide(playerid, ATRP_Injured[playerid][8]);
...
Function: UpdateRusunTime()
{
	...
	if(currentTime >= g_RusunTime)
	{
		foreach(new rsid : Rusun)
		{
			...
			mysql_tquery(mdb_query, sprintf("UPDATE `player_characters` SET `Char_HasRusunID`=-1 WHERE `pID`=%d", RusunData[rsid][rusunOwnerID]));
			...
			new query[258];
			mysql_format(mdb_query, query, sizeof(query), "UPDATE `stuffs` SET `globaltime_rusun`=%d WHERE `ID`=0", g_RusunTime);
			mysql_tquery(mdb_query, query);
		}
	}
	return 1;
...
ptask GiveGovermentAllowance[900000](playerid)
{
	...
	if(GetPlayerVIPLevel(playerid) == 1)
	{
		...
		Info(playerid, "Anda pengguna VIP Ranger Kuning, anda mendapatkan tambahan XP 125.");
		GivePlayerXP(playerid, 125);
	...
	else if(GetPlayerVIPLevel(playerid) == 2)
	{
		...
		Info(playerid, "Anda pengguna VIP Ranger Hijau, anda mendapatkan tambahan XP 155.");
		GivePlayerXP(playerid, 155);
		PlayerPlaySound(playerid, 1139, 0.0, 0.0, 0.0);
	}
	else if(GetPlayerVIPLevel(playerid) == 3)
	{
		...
		Info(playerid, "Anda pengguna VIP Ranger Merah, anda mendapatkan tambahan XP 1000.");
		GivePlayerXP(playerid, 1000);
		PlayerPlaySound(playerid, 1139, 0.0, 0.0, 0.0);
	}
	else 
...
Blacklist_Check(playerid, const type[], target[])
{
	...
	execute = mysql_query(mdb_query, sprintf("SELECT * FROM `player_bans` WHERE `%s` = '%s' LIMIT 1;", type, target));
	...
	if(cache_num_rows())
	{
		...
		if(time != 0 && time <= currentTime) // melepas status banned akun
		{
			...
			mysql_tquery(mdb_query, sprintf("SELECT * FROM `player_characters` WHERE `Char_Name` = '%s' LIMIT 1;", pbanname), "LoadPlayerData", "d", playerid);
		}
		else
		{
			new PlayerIP[16];
			GetPlayerIp(playerid, PlayerIP, sizeof(PlayerIP));
			mysql_tquery(mdb_query, sprintf("UPDATE `player_bans` SET `last_activity_timestamp` = '%d' WHERE `name` = '%s'", gettime(), AccountData[playerid][pUCP]));
...
{
	mysql_tquery(mdb_query, sprintf("DELETE FROM `player_bans` WHERE `name` = '%s';", SQL_ReturnEscaped(username)));
	return 1;
}
...
Blacklist_UCPExists(const type[], const target[])
{
	...
	execute = mysql_query(mdb_query, sprintf("SELECT `%s` FROM `player_characters` WHERE `%s` = '%s' LIMIT 1;", type, type, target));
...
Blacklist_Exists(const type[], const target[])
{
	...
	execute = mysql_query(mdb_query, sprintf("SELECT `%s` FROM `player_bans` WHERE `%s` = '%s' LIMIT 1;", type, type, target));
