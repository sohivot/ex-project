...
{
    new shstr[200];
    mysql_format(mdb_query, shstr, sizeof(shstr), "SELECT * FROM `invoices` WHERE `Owner`=%d ORDER BY Cost DESC", AccountData[targetid][pID]);
    new Cache:execute = mysql_query(mdb_query, shstr);
    if(cache_num_rows() > 0)
    {
...
{
    new shstr[200];
    mysql_format(mdb_query, shstr, sizeof(shstr), "SELECT * FROM `invoices` WHERE `Owner`=%d ORDER BY Cost DESC", AccountData[playerid][pID]);
    new Cache:execute = mysql_query(mdb_query, shstr);
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    else if(dialogid == DIALOG_INVOICE_COST)
    {
        ...
        mysql_format(mdb_query, cQuery, sizeof(cQuery), "INSERT INTO `invoices` SET `Name`='%s', `Sender`=%d, `Faction`=%d, `Cost`=%d, `Owner`=%d", invoicename, AccountData[playerid][pID], AccountData[playerid][pFaction], cost, AccountData[targetid][pID]);
        mysql_tquery(mdb_query, cQuery, "OnInvoiceCreated", "dd", playerid, targetid);
    }
    else if(dialogid == DIALOG_PAY_INVOICE)
    {
        ...
        new cQuery[200];
        mysql_format(mdb_query, cQuery, sizeof(cQuery), "SELECT * FROM `invoices` WHERE `Owner`=%d ORDER BY Cost DESC", AccountData[playerid][pID]);
        new Cache:execute = mysql_query(mdb_query, cQuery);
        if(cache_num_rows() > 0)
        {
            ...
            if(listitem >= 0 && listitem < cache_num_rows())
            {
                ...
                if(factid == 1) // Polisi Faction ID
                {
                    ...
                    foreach(new i : Player)
                    {
                        ...
                        {
                            AccountData[i][pBankMoney] += (cost/100)*10;
                    ...
                    mysql_format(mdb_query, rasberry, sizeof(rasberry), "UPDATE `player_characters` SET `Char_BankMoney` = `Char_BankMoney` + %d WHERE `pID` = %d", (cost/100)*60, sender);
                    mysql_tquery(mdb_query, rasberry);

                    AccountData[playerid][pBankMoney] -= cost;
                    PoliceMoneyVault += (cost/100)*90; // Penghasilan 20% untuk masuk ke brankas faction tersebut!

                    ShowTDN(playerid, NOTIFICATION_SUKSES, sprintf("Berhasil membayar invoice sebesar ~g~%s", FormatMoney(cost)));
                    
                    mysql_format(mdb_query, sha, sizeof(sha), "DELETE FROM `invoices` WHERE `invoiceID`=%d", invid);
                    mysql_tquery(mdb_query, sha);

                    mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `stuffs` SET `policemoneyvault`=%d WHERE `ID`=0", PoliceMoneyVault);
                ...
                else if(factid == 2) // Pemerintah Faction ID
                {
                    ...
                    mysql_format(mdb_query, rasberry, sizeof(rasberry), "UPDATE `player_characters` SET `Char_BankMoney` = `Char_BankMoney` + %d WHERE `pID` = %d", (cost/100)*30, sender);
                    mysql_tquery(mdb_query, rasberry);

                    AccountData[playerid][pBankMoney] -= cost;
                    PemerintahMoneyVault += (cost/100)*90; // Penghasilan 20% untuk masuk ke brankas faction tersebut!
                    ...
                    mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `stuffs` SET `pemerintahmoneyvault`=%d WHERE `ID`=0", PemerintahMoneyVault);
                    mysql_tquery(mdb_query, icsr);
                }
                else if(factid == 3) // EMS Faction ID
                {
                    ...
                    mysql_format(mdb_query, rasberry, sizeof(rasberry), "UPDATE `player_characters` SET `Char_BankMoney` = `Char_BankMoney` + %d WHERE `pID` = %d", (cost/100)*20, sender);
                    mysql_tquery(mdb_query, rasberry);

                    AccountData[playerid][pBankMoney] -= cost;
                    EMSMoneyVault += (cost/100)*90; // Penghasilan 20% untuk masuk ke brankas faction tersebut!
                    ...
                    mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `stuffs` SET `emsmoneyvault`=%d WHERE `ID`=0", EMSMoneyVault);
                    mysql_tquery(mdb_query, icsr);
                }
                else if(factid == 5) // Bengkel Faction ID
                {
                    ...
                    AccountData[playerid][pBankMoney] -= cost;
                    BengkelMoneyVault += (cost/100)*90; // Penghasilan 20% untuk masuk ke brankas faction tersebut!
                    ...
                    mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `stuffs` SET `bengkelmoneyvault`=%d WHERE `ID`=0", BengkelMoneyVault);
                    mysql_tquery(mdb_query, icsr);
                }
                else if(factid == 6) // Pedagang Faction ID
                {
                    ...
                    AccountData[playerid][pBankMoney] -= cost;
                    RestoMoneyVault += (cost/100)*90; // Penghasilan 20% untuk masuk ke brankas faction tersebut!
                    ...
                    mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `stuffs` SET `restomoneyvault`=%d WHERE `ID`=0", RestoMoneyVault);
                    mysql_tquery(mdb_query, icsr);
                }
            }
        }
