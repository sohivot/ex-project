...
Function: DisplayFactionMemberList(playerid, pageindex)
{
    ...
    mysql_tquery(mdb_query, sprintf("SELECT `pID` FROM `player_characters` WHERE `Char_Faction` = %i", MemberBrowsing[playerid]), "GetTotalFactionMemberCount", "isi", playerid, string, onlineCount);
    return 1;
}
...
Dialog:FactionMemberList(playerid, response, listitem, inputtext[])
{
    ...
    if(strcmp(inputtext, "> Halaman Selanjutnya", inputtext, true) == 0)
    {
        ...
        mysql_tquery(mdb_query, string, "DisplayFactionMemberList", "dd", playerid, MemberListIndex[playerid]);
    }
