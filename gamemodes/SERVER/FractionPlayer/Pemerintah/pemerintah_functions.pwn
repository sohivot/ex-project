...
VarsDoorPemerintah()
{
    PemerStuff[PemerDuty] = CreateDynamicSphere(1379.173095, 1574.956787, 17.000314, 2.0, 2, 1, -1);
    PemerStuff[PemerBrankas] = CreateDynamicSphere(1369.908935, 1597.699951, 15.670308, 2.0, 2, 1, -1);
    PemerStuff[PemerLocker] = CreateDynamicSphere(1372.754882, 1615.726806, 15.670307, 2.0, 2, 1, -1);
    PemerStuff[PemerDesk] = CreateDynamicSphere(1386.678955, 1606.277954, 15.670304, 2.0, 2, 1, -1);
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    switch(dialogid)
    {
        ...
        case DIALOG_PEMERVAULT:
        {
            ...
            switch(listitem)
            {
                ...
                case 1: //withdraw
                {
                    ...
                    format(str, sizeof(str), "Nama Item\tJumlah\tBerat (-/-)\n");
                    mysql_query(mdb_query, "SELECT * FROM `pemerintah_brankas` WHERE `PID`=0");
                    if(cache_num_rows() > 0)
                    {
        ...
        case DIALOG_PEMERVAULT_IN:
        {
            ...
            new invstr[1028];
            mysql_format(mdb_query, shstr, sizeof(shstr), "SELECT * FROM `pemerintah_brankas` WHERE `PID`=0 AND `Item` = '%s'", FactionBrankas[playerid][factionBrankasTemp]);
            mysql_query(mdb_query, shstr);
            new rows = cache_num_rows();
            if(rows > 0)
            {
                mysql_format(mdb_query, invstr, sizeof(invstr), "UPDATE `pemerintah_brankas` SET `Quantity` = `Quantity` + %d WHERE `PID` = 0 AND `Item`='%s'", quantity, FactionBrankas[playerid][factionBrankasTemp]);
                mysql_tquery(mdb_query, invstr, "OnPemerintahDeposit", "i", playerid);
            }
            else 
            {
                mysql_format(mdb_query, invstr, sizeof(invstr), "INSERT INTO `pemerintah_brankas` SET `PID`=0, `Item`='%s', `Model`=%d, `Quantity`=%d", FactionBrankas[playerid][factionBrankasTemp], FactionBrankas[playerid][factionBrankasModel], quantity);
                mysql_tquery(mdb_query, invstr, "OnPemerintahDeposit", "i", playerid);
            }
            AccountData[playerid][menuShowed] = false;
        }
        ...
        case DIALOG_PEMERVAULT_OUT:
        {
            ...
            {
                mysql_format(mdb_query, jts, sizeof(jts), "UPDATE `pemerintah_brankas` SET `Quantity`=%d WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasQuant], FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(mdb_query, jts);
            }
            else 
            {
                mysql_format(mdb_query, jts, sizeof(jts), "DELETE FROM `pemerintah_brankas` WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(mdb_query, jts);
            }
        ...
        case DIALOG_PEMERINTAH_INVITE:
        {
            ...
            mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Faction` = 2, `Char_FactionRank` = 1 WHERE `pID` = %d", AccountData[targetid][pID]);
        ...
        case DIALOG_PEMERKICKMEMBER:
        {
            ...
            if(cache_num_rows())
            {
                ...
                new strgbg[158];
                mysql_format(mdb_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d AND `PVeh_Faction` = 2", pidrow);
                mysql_tquery(mdb_query, strgbg);
...
CMD:ubahnamaktp(playerid, params[])
{
    ...
    new query[255];
    mysql_format(mdb_query, query, sizeof(query), "SELECT `Char_Name` FROM `player_characters` WHERE `Char_Name`='%s'", tmp);
    mysql_tquery(mdb_query, query, "OnChangeKtpName", "iis", otherid, playerid, tmp);
    return 1;
}
...
CMD:makektp(playerid, params[])
{
    ...
    AccountData[userid][KtpTime] = gettime() + (7 * 86400);
