...
stock PemerintahItem_Add(facid, item[], model, quantity = 1)
{
	...
	if (itemid == -1)
	{
	    ...
	    if (itemid != -1)
	    {
			...
			mysql_format(mdb_query, string, sizeof(string), "INSERT INTO `brankas_pemerintah` (`ID`, `ItemName`, `ItemModel`, `ItemQuantity`) VALUES('%d', '%s', '%d', '%d')", facid, item, model, quantity);
			mysql_tquery(mdb_query, string, "OnPemerintahAddItem", "dd", facid, itemid);
	...
	{
	    mysql_format(mdb_query, string, sizeof(string), "UPDATE `brankas_pemerintah` SET `ItemQuantity` = `ItemQuantity` + %d WHERE `ID` = '%d' AND `ItemID` = '%d'", quantity, facid, PemerintahStorage[facid][itemid][ItemID]);
...
stock PemerintahItem_Remove(facid, item[], quantity = 1)
{
	...
	if (itemid != -1)
	{
		...
		if (quantity == -1 || PemerintahStorage[facid][itemid][ItemQuantity] < 1)
		{
		    ...
		    mysql_format(mdb_query, string, sizeof(string), "DELETE FROM `brankas_pemerintah` WHERE `ID` = '%d' AND `ItemID` = '%d'", facid, PemerintahStorage[facid][itemid][ItemID]);
	        mysql_tquery(mdb_query, string);
		}
