...
Families_Save(famid)
{
    ...
    F_GarageX=%f, F_GarageY=%f, F_GarageZ=%f,  F_GarageSpawnX=%f, F_GarageSpawnY=%f, F_GarageSpawnZ=%f, F_GarageSpawnA=%f, F_GarageInterior=%d, F_GarageWorld=%d, F_Type=%d, F_Tier=%d WHERE F_ID='%d'",
...
Families_Refresh(id)
{
    if(id != -1)
    {
        ...
        {
            FamData[id][famBrankasLabel] = CreateDynamic3DTextLabel(""YELLOW"[ALT]"WHITE" Brankas", -1, FamData[id][famBrankasPos][0], FamData[id][famBrankasPos][1],FamData[id][famBrankasPos][2] + 0.25, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, id, FamData[id][famInterior], -1);
        }

        if(FamData[id][famBosDeskPos][0] != 0.0 && FamData[id][famBosDeskPos][1] != 0.0 && FamData[id][famBosDeskPos][2] != 0.0)
        {
            FamData[id][famBosDeskLabel] = CreateDynamic3DTextLabel(""YELLOW"[ALT]"WHITE" Families Desk", -1, FamData[id][famBosDeskPos][0], FamData[id][famBosDeskPos][1], FamData[id][famBosDeskPos][2] + 0.25, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, id, FamData[id][famInterior], -1);
        }

        if(FamData[id][famgaragePos][0] != 0.0)
        {
            FamData[id][famgarageObject] = CreateCirclePickup(0xF5DEB3FF, FamData[id][famgaragePos][0], FamData[id][famgaragePos][1], FamData[id][famgaragePos][2], FamData[id][famgarageWorld], FamData[id][famgarageInt], -1); 
...
public Families_Load()
{
    ...
    if(rows)
    {
        ...
        while(i < rows)
        {
            ...
            cache_get_value_int(i, "F_Type", FamData[fid][famType]);
            cache_get_value_int(i, "F_Tier", FamData[fid][famTier]);
...
CMD:setfamilies(playerid, params[])
{
    if(AccountData[playerid][pCharStory] < 0)
		return Error(playerid, "player belum ada karakter story");
    if(AccountData[playerid][pAdminBS] < 1)
...
CMD:famcreate(playerid, params[])
{
    ...
    FamData[id][famType] = 0;
    FamData[id][famTier] = 0;
    ...
    new icsr[200];
    mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Family`=%d, `Char_FamilyRank`=6 WHERE `pID`=%d", id, AccountData[otherid][pID]);
    mysql_tquery(mdb_query, icsr);

    mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `families` SET `F_ID` = '%d', `F_Name` = '%s', `F_Leader` = '%s'", id, name, AccountData[otherid][pName], AccountData[otherid][pFamilyType]);
	mysql_tquery(mdb_query, query, "OnFamiliesCreated", "d", id);
	return 1;
}
...
CMD:famedit(playerid, params[])
{
    ...
    if(sscanf(params, "ds[24]S()[128]", fid, type, string))
    {
        ...
        Syntax(playerid, "/famedit [id] [entinity] (location, interior, name, leader, bosdesk, brankas, money, garage, garagespawn, type, tier)");
        return 1;
    }
    ...
    else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", type))
            return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/famedit [id] [type] [1.Mafia 2.Gangster 3.MC 4.JDM 5.Kartel]");

        if(type < 1 || type > 5)
            return ShowTDN(playerid, NOTIFICATION_ERROR, "Type yang anda input tidak valid!");
        
        FamData[fid][famType] = type;
        Families_Save(fid);
        Families_Refresh(fid);

        SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah telah mengubah type Families: %s menjadi %s", GetAdminName(playerid), FamData[fid][famName], GetFamTypeName(fid));
    }
    else if(!strcmp(type, "tier", true))
    {
        new tier;
        if(sscanf(string, "d", tier))
            return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/famedit [id] [tier] [0.Official 1.Verified]");

        if(tier < 0 || tier > 1)
            return ShowTDN(playerid, NOTIFICATION_ERROR, "Tier yang anda input tidak valid!");
        
        FamData[fid][famTier] = tier;
        Families_Save(fid);
        Families_Refresh(fid);

        SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah telah mengubah tier Families: %s menjadi %s", GetAdminName(playerid), FamData[fid][famName], tier ? "Verified" : "Official");
    }
    return 1;
}
...
CMD:famdelete(playerid, params[])
{
    ...
    mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Family`=-1, `Char_FamilyRank`=0 WHERE `Char_Family`=%d", fid);
    mysql_tquery(mdb_query, query);

    mysql_format(mdb_query, query, sizeof(query), "DELETE FROM families WHERE F_ID ='%d'", fid);
    mysql_tquery(mdb_query, query);
...
GetFamTypeName(id)
{
	...
	format(frmtname, sizeof(frmtname), "%s", FamTypeName[id]);
	return frmtname;
}
...
{
    new id, famname[126], famleader[MAX_PLAYER_NAME], famonline, type, tier, Cache:execute;
    execute = mysql_query(mdb_query, sprintf("SELECT `F_ID`, `F_Name`, `F_Leader`, `F_Type`, `F_Tier` FROM `families` ORDER BY `F_ID` ASC"), true);
    if(cache_num_rows())
    {
        new list[2500];

        format(list, sizeof(list), "FID\tName [Type] - Tier\tOnline\tLeader\n");
        for(new i = 0; i < cache_num_rows(); i ++)
        {
            cache_get_value_name_int(i, "F_ID", id);
            cache_get_value_name(i, "F_Name", famname);
            cache_get_value_name(i, "F_Leader", famleader);
            cache_get_value_name_int(i, "F_Type", type);
            cache_get_value_name_int(i, "F_Tier", tier);
...
hook OnScriptInit()
{
    ...
    Families_Stuffs[BlackmarketArea] = CreateDynamicSphere(-1477.560668, 2629.999511, 58.787948, 2.0, 0, 0, -1, 0);
    Families_Stuffs[BlackmarketLabel] = CreateDynamic3DTextLabel(""RED"[ALT]"WHITE" Blackmarket", -1, -1477.560668, 2629.999511, 58.787948, 15.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0, 0, -1, 15.0, -1, 0);
...
hook OnPlayerEnterDynArea(playerid, STREAMER_TAG_AREA:areaid)
{
    ...
    {
        ShowKey(playerid, "[ALT] Jual Marijuana");
    }

    if(areaid == Families_Stuffs[WashRedMoney])
    {
        ShowKey(playerid, "[ALT] Cuci Uang");
    }
    ...
    foreach(new i : Families) if (areaid == FamData[i][famgarageArea])
    {
        ...
        {
            ShowKey(playerid, "[ALT] Garasi Families");
        }
    }
    return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if(IsPlayerInDynamicArea(playerid, Families_Stuffs[WashRedMoney]))
        {
            ...
            if(countpd <= 3 && countems <= 1) return ShowTDN(playerid, NOTIFICATION_ERROR, "Minimal 4 Polisi Duty dan 2 EMS Duty!");
        ...
        if(IsPlayerInDynamicArea(playerid, Families_Stuffs[BlackmarketArea]))
        {
            ...
            \nTec 9\t"RED"$22,500\
            \n"GRAY"COLT\t"RED"$10,500","Pilih", "Batal");
        }
    ...
    switch(dialogid)
    {
        ...
        case DIALOG_BLACKMARKET:
        {
            ...
            switch(listitem)
            {
                case 0: // DE
                {
                    if(AccountData[playerid][pRedMoney] < 14250) return ShowTDN(playerid, NOTIFICATION_ERROR, "Uang merah tidak mencukupi!");
                    AccountData[playerid][pRedMoney] -= 14250;
                    GivePlayerWeaponEx(playerid, 24, 57);
                    ShowItemBox(playerid, "Removed $14,250", "Uang Merah", 1212);
                    ShowItemBox(playerid, "Received 1x", "Desert Eagle", 348);
                }
                case 1: // Shotgun
                {
                    if(AccountData[playerid][pRedMoney] < 18250) return ShowTDN(playerid, NOTIFICATION_ERROR, "Uang merah tidak mencukupi!");
                    AccountData[playerid][pRedMoney] -= 18250;
                    GivePlayerWeaponEx(playerid, 25, 100);
                    ShowItemBox(playerid, "Removed $18,250", "Uang Merah", 1212);
                    ShowItemBox(playerid, "Received 1x", "Shotgun", 349);
                }
                case 2: //TEC 9
                {
                    if(AccountData[playerid][pRedMoney] < 22500) return ShowTDN(playerid, NOTIFICATION_ERROR, "Uang merah tidak mencukupi!");
                    AccountData[playerid][pRedMoney] -= 22500;
                    GivePlayerWeaponEx(playerid, 32, 150);
                    ShowItemBox(playerid, "Removed $22,500", "Uang Merah", 1212);
                    ShowItemBox(playerid, "Received 1x", "TEC 9", 372);
                }/*
                case 3: //MP 5
                {
                    if(AccountData[playerid][pRedMoney] < 10000) return ShowTDN(playerid, NOTIFICATION_ERROR, "Uang merah tidak mencukupi!");
                    AccountData[playerid][pRedMoney] -= 10000;
                    GivePlayerWeaponEx(playerid, 29, 250);
                    ShowItemBox(playerid, "Removed $10,000", "Uang Merah", 1212);
                ...
                case 3: //colt
                {
                    if(AccountData[playerid][pRedMoney] < 10500) return ShowTDN(playerid, NOTIFICATION_ERROR, "Uang merah tidak mencukupi!");
                    AccountData[playerid][pRedMoney] -= 10500;
                    GivePlayerWeaponEx(playerid, 22, 67);
                    ShowItemBox(playerid, "Removed $10,500", "Uang Merah", 1212);
                    ShowItemBox(playerid, "Received 1x", "COLT", 346);
                }   
                case 4: //Hacking Device
                {
                    if(AccountData[playerid][pRedMoney] < 10500) return ShowTDN(playerid, NOTIFICATION_ERROR, "Uang merah tidak mencukupi!");
                    AccountData[playerid][pRedMoney] -= 10500;
                    ShowItemBox(playerid, "Removed $10,500", "Uang Merah", 1212);
                    if(Inventory_Count(playerid, "Hacking Device") >= 3) return ShowTDN(playerid, NOTIFICATION_ERROR, "Armour Kamu Di Tas Sudah Banyak!");
                    Inventory_Add(playerid, "Hacking Device", 19273, 1);
                    ShowItemBox(playerid, "Received 1x", "Hacking Device", 19273);
                }                                                             
            }
        }
        case DIALOG_FAMSTAKE_MONEY:
        {
            ...
            Warning(targetid, "(Player Name: %s) telah mengambil uang anda sebanyak "RED"%s", AccountData[playerid][pName], FormatMoney(value));
        ...
        case DIALOG_FAMSVAULT:
        {
            ...
            switch(listitem)
            {
                ...
                case 1: //withdraw
                {
                    ...
                    mysql_format(mdb_query, icsr, sizeof(icsr), "SELECT * FROM `badside_brankas` WHERE `ID`=%d", AccountData[playerid][pFamily]);
                    mysql_query(mdb_query, icsr);
                    if(cache_num_rows() > 0)
                    {
        ...
        case DIALOG_FAMSVAULT_IN:
        {
            ...
            new invstr[1028];
            mysql_format(mdb_query, shstr, sizeof(shstr), "SELECT * FROM `badside_brankas` WHERE `ID`=%d AND `fItemName`='%s'", AccountData[playerid][pFamily], FactionBrankas[playerid][factionBrankasTemp]);
            mysql_query(mdb_query, shstr);
            new rows = cache_num_rows();
            if(rows > 0)
            {
                mysql_format(mdb_query, invstr, sizeof(invstr), "UPDATE `badside_brankas` SET `fItemQuantity` = `fItemQuantity` + %d WHERE `ID`=%d AND `fItemName`='%s'", quantity, AccountData[playerid][pFamily], FactionBrankas[playerid][factionBrankasTemp]);
                mysql_tquery(mdb_query, invstr, "OnBadsideDeposit", "i", playerid);
            }
            else 
            {
                mysql_format(mdb_query, invstr, sizeof(invstr), "INSERT INTO `badside_brankas` SET `ID`=%d, `fItemName`='%s', `fItemModel`=%d, `fItemQuantity`=%d", AccountData[playerid][pFamily], FactionBrankas[playerid][factionBrankasTemp], FactionBrankas[playerid][factionBrankasModel], quantity);
                mysql_tquery(mdb_query, invstr, "OnBadsideDeposit", "i", playerid);
            }
            AccountData[playerid][menuShowed] = false;
        }
        case DIALOG_FAMSVAULT_WITHDRAW:
        {
            ...
            new tss[188];
            mysql_format(mdb_query, tss, sizeof(tss), "SELECT * FROM `badside_brankas` WHERE `ID`=%d", AccountData[playerid][pFamily]);
            mysql_query(mdb_query, tss);
            if(cache_num_rows() > 0)
            {
        ...
        case DIALOG_FAMSVAULT_OUT:
        {
            ...
            {
                mysql_format(mdb_query, jts, sizeof(jts), "UPDATE `badside_brankas` SET `fItemQuantity`=%d WHERE `fItemID`=%d", FactionBrankas[playerid][factionBrankasQuant], FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(mdb_query, jts);
            }
            else 
            {
                mysql_format(mdb_query, jts, sizeof(jts), "DELETE FROM `badside_brankas` WHERE `fItemID`=%d", FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(mdb_query, jts);
            }
        ...
        case DIALOG_FAMILIESSETRANK:
        {
            ...
            mysql_query(mdb_query, sprintf("SELECT * FROM `player_characters` WHERE `Char_Family` = %d ORDER BY `Char_FamilyRank` DESC", famid));
            new rows = cache_num_rows();
            if(rows)
            {
        ...
        case DIALOG_FAMILIESKICKMEMBER:
        {
            ...
            if(cache_num_rows())
            {
                ...
                mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Family`=-1, `Char_FamilyRank`=0 WHERE `pID`=%d", pidrow);
                mysql_tquery(mdb_query, icsr);
        ...
        case DIALOG_RANK_SET_FAMILIES:
        {
            ...
            new affah[128];
            mysql_format(mdb_query, affah, sizeof(affah), "UPDATE `player_characters` SET `Char_FamilyRank`=%d WHERE `pID`=%d", strval(inputtext), AccountData[playerid][pTempSQLFamMemberID]);
...
Dialog:Bosdesk_Family(playerid, response, listitem, inputtext[])
{
    ...
    switch(listitem)
    {
        ...
        {
            mysql_query(mdb_query, sprintf("SELECT * FROM player_characters WHERE Char_Family = %d ORDER BY Char_FamilyRank DESC", AccountData[playerid][pFamily]));
...
Dialog:FamiliesKeuangan(playerid, response, listitem, inputtext[])
{
    ...
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            new frmxt[268];
            format(frmxt, sizeof(frmxt), "Mengambil %d %s dari brankas families", FactionBrankas[playerid][factionBrankasTemp]);
            AddFamiliesLog(AccountData[playerid][pName], AccountData[playerid][pUCP], frmxt, GetFamiliesName(playerid));            
        }
        else if(!strcmp(option, "depo", true))
        {
            ...
            new frmxt[268];
            format(frmxt, sizeof(frmxt), "Memasukan %d %s dari brankas families", FactionBrankas[playerid][factionBrankasTemp]);
            AddFamiliesLog(AccountData[playerid][pName], AccountData[playerid][pUCP], frmxt, GetFamiliesName(playerid));  
        }
    }
    return 1;
...
Dialog:FamiliesInvite(playerid, response, listitem, inputtext[])
{
    ...
    {
        if(AccountData[playerid][pCharStory] < 0)
		    return ShowTDN(playerid, NOTIFICATION_ERROR, "player belum ada karakter story");
        ...
        new icsr[200];
        mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Family`=%d, `Char_FamilyRank`=1 WHERE `pID`=%d", AccountData[playerid][pFamily], AccountData[id][pID]);
        mysql_tquery(mdb_query, icsr);
    }
    return 1;
}
...
public SellMarjun(playerid)
{
    ...
    //    GivePlayerMoneyEx(playerid, pay);
        AccountData[playerid][pRedMoney] += pay;
        ShowItemBox(playerid, sprintf("Removed %dx", total), "MARIJUANA", 1575);
        ShowItemBox(playerid, sprintf("Received %s", FormatMoney(pay)), "Uang Merah", 1212);
    }
    else 
    {
