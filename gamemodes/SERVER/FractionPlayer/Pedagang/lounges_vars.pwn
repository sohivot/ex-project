...
hook OnGameModeInit()
{
    Pedagang_Stuff[PdgDuty] = CreateDynamicSphere(652.100097, -1870.214721, 6.515146, 1.5, 0, 0, -1);
    Pedagang_Stuff[PdgLocker] = CreateDynamicSphere(670.213928, -1880.057495, 6.259113, 1.5, 0, 0, -1);
    Pedagang_Stuff[PdgGarasi] = CreateDynamicSphere(640.879150, -1863.576782, 6.065148, 1.5, 0, 0, -1);
    Pedagang_Stuff[PdgCooking] = CreateDynamicSphere(657.255004, -1870.239990, 6.525146, 1.5, 0, 0, -1);
    Pedagang_Stuff[PdgDesk] = CreateDynamicSphere(657.953857, -1870.976074, 10.575151, 1.5, 0, 0, -1);
    Pedagang_Stuff[PdgKulkas] = CreateDynamicSphere(665.814575, -1883.140747, 6.259114, 1.5, 0, 0, -1);
    ...
    Pedagang_Stuff[PdggaragePos][0] = 640.879150;
    Pedagang_Stuff[PdggaragePos][1] = -1863.576782;
    Pedagang_Stuff[PdggaragePos][2] = 6.065148;

    Pedagang_Stuff[pdggarageSpawnPos][0] = 641.886230;
    Pedagang_Stuff[pdggarageSpawnPos][1] = -1855.175659;
    Pedagang_Stuff[pdggarageSpawnPos][2] = 5.800396;
    Pedagang_Stuff[pdggarageSpawnPos][3] = 354.599334;
}
...
Dialog:SaldoPedagangDeposit(playerid, response, listitem, inputtext[])
{
    ...
    if(response)
    {
        ...
        if(!strcmp(string, "depo", true))
        {
            ...
            new query[255];
            mysql_format(mdb_query, query, sizeof(query), "UPDATE `stuffs` SET `restomoneyvault` = %d WHERE `ID` = 0", RestoMoneyVault);
...
Dialog:Pedagang_Invite(playerid, response, listitem, inputtext[])
{
    ...
    mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Faction`= 6, `Char_FactionRank` = 1 WHERE `pID`=%d", AccountData[targetid][pID]);
    ...
    switch(dialogid)
    {
        ...
        case DIALOG_PEDKICKMEMBER:
        {
            ...
            if(cache_num_rows())
            {
                ...
                new strgbg[158];
                mysql_format(mdb_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d AND `PVeh_Faction` = 6", pidrow);
                mysql_tquery(mdb_query, strgbg);
