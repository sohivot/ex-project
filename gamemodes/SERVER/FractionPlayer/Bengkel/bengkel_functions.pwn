...
VarsGerbangBengkel()
{
    ...
    CreateDynamic3DTextLabel(string, -1, -74.524, -1541.421, 2.889 + 0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0, -1);
    CreateDynamic3DTextLabel(string, -1, -82.360, -1548.545, 2.889 + 0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0, -1);
    CreateDynamic3DTextLabel(string, -1, -89.708, -1555.788, 2.889 + 0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0, -1);
    CreateDynamic3DTextLabel(string, -1, -97.682, -1563.331, 2.889 + 0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0, -1);
    CreateDynamicPickup(1239, 23, -74.524, -1541.421, 2.889 + 0.1, -1, -1, -1, 10.0);
    CreateDynamicPickup(1239, 23, -82.360, -1548.545, 2.889 + 0.1, -1, -1, -1, 10.0);
    CreateDynamicPickup(1239, 23, -89.708, -1555.788, 2.889 + 0.1, -1, -1, -1, 10.0);
    CreateDynamicPickup(1239, 23, -97.682, -1563.331, 2.889 + 0.1, -1, -1, -1, 10.0);

    Bengkel_Stuff[BengkelDuty] = CreateDynamicSphere(-75.594, -1594.967, 2.685, 1.5, -1, -1, -1);//duty
    Bengkel_Stuff[BengkelDesk] = CreateDynamicSphere(-70.734, -1539.141, 10.605, 1.5, -1, -1, -1);
    Bengkel_Stuff[BengkelLocker] = CreateDynamicSphere(-93.592, -1562.046, 10.603, 1.5, -1, -1, -1);
    Bengkel_Stuff[BengkelGarage] = CreateDynamicSphere(-84.8240, 1043.5159, 19.7411, 1.5, -1, -1, -1);
    Bengkel_Stuff[BengkelBrankas] = CreateDynamicSphere(-104.537, -1577.331, 10.604, 1.5, -1, -1, -1);
    Bengkel_Stuff[BengkelCraft] = CreateDynamicSphere(-103.282, -1570.368, 10.603, 1.5, -1, -1, -1);
    Bengkel_Stuff[BengkelModif][0] = CreateDynamicSphere(-74.524, -1541.421, 2.889, 2.5, -1, -1, -1);
    Bengkel_Stuff[BengkelModif][1] = CreateDynamicSphere(-82.360, -1548.545, 2.889, 2.5, -1, -1, -1);
    Bengkel_Stuff[BengkelModif][2] = CreateDynamicSphere(-89.708, -1555.788, 2.889, 2.5, -1, -1, -1);
    Bengkel_Stuff[BengkelModif][3] = CreateDynamicSphere(-97.682, -1563.331, 2.889, 2.5, -1, -1, -1);
    ...
    Bengkel_Stuff[bengkelgaragePos][0] = -69.549; // -69.549, -1592.769, 2.617
    Bengkel_Stuff[bengkelgaragePos][1] = -1592.769;
    Bengkel_Stuff[bengkelgaragePos][2] = 2.617;
    
    Bengkel_Stuff[bengkelgarageSpawnPos][0] = -54.701; //spawn car -54.701, -1585.105, 2.493, 7.469
    Bengkel_Stuff[bengkelgarageSpawnPos][1] = -1585.105;
    Bengkel_Stuff[bengkelgarageSpawnPos][2] = 2.493;
    Bengkel_Stuff[bengkelgarageSpawnPos][3] = 7.469;
}
...
hook OnPlayerEnterDynArea(playerid, STREAMER_TAG_AREA:areaid)
{
    if(AccountData[playerid][pFaction] == FACTION_BENGKEL)
    {
        ...
        {
            ShowKey(playerid, "[ALT] Locker Bengkel");
        }
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    switch(dialogid)
    {
        ...
        case DIALOG_BENGVAULT:
        {
            ...
            switch(listitem)
            {
                ...
                case 1: //withdraw
                {
                    ...
                    format(str, sizeof(str), "Nama Item\tJumlah\tBerat (-/-)\n");
                    mysql_query(mdb_query, "SELECT * FROM `bengkel_brankas` WHERE `PID`=0");
                    if(cache_num_rows() > 0)
                    {
        ...
        case DIALOG_BENGVAULT_IN:
        {
            ...
            new invstr[1028];
            mysql_format(mdb_query, shstr, sizeof(shstr), "SELECT * FROM `bengkel_brankas` WHERE `PID`=0 AND `Item` = '%s'", FactionBrankas[playerid][factionBrankasTemp]);
            mysql_query(mdb_query, shstr);
            new rows = cache_num_rows();
            if(rows > 0)
            {
                mysql_format(mdb_query, invstr, sizeof(invstr), "UPDATE `bengkel_brankas` SET `Quantity` = `Quantity` + %d WHERE `PID` = 0 AND `Item`='%s'", quantity, FactionBrankas[playerid][factionBrankasTemp]);
                mysql_tquery(mdb_query, invstr, "OnBengkelDeposit", "i", playerid);
            }
            else 
            {
                mysql_format(mdb_query, invstr, sizeof(invstr), "INSERT INTO `bengkel_brankas` SET `PID`=0, `Item`='%s', `Model`=%d, `Quantity`=%d", FactionBrankas[playerid][factionBrankasTemp], FactionBrankas[playerid][factionBrankasModel], quantity);
                mysql_tquery(mdb_query, invstr, "OnBengkelDeposit", "i", playerid);
            }
            AccountData[playerid][menuShowed] = false;
        }
        ...
        case DIALOG_BENGVAULT_OUT:
        {
            ...
            {
                mysql_format(mdb_query, jts, sizeof(jts), "UPDATE `bengkel_brankas` SET `Quantity`=%d WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasQuant], FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(mdb_query, jts);
            }
            else 
            {
                mysql_format(mdb_query, jts, sizeof(jts), "DELETE FROM `bengkel_brankas` WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(mdb_query, jts);
            }
        ...
        case DIALOG_BENGKELKICKMEMBER:
        {
            ...
            if(cache_num_rows())
            {
                ...
                new strgbg[158];
                mysql_format(mdb_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d AND `PVeh_Faction` = 5", pidrow);
        ...
        case DIALOG_BENGKEL_INVITE:
        {
            ...
            mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Faction`= 5, `Char_FactionRank` = 1 WHERE `pID`=%d", AccountData[targetid][pID]);
            mysql_tquery(mdb_query, icsr);
        ...
        case DIALOG_BENGKELDELCAR:
        {
            ...
            if(cache_num_rows())
            {
                ...
                if(listitem >= 0 && listitem < cache_num_rows())
                {
                    ...
                    mysql_format(mdb_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `id` = %d", hapvid);
                    mysql_tquery(mdb_query, strgbg);
