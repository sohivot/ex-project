...
VarsDoorEms()
{
    EMSStuff[EMSDuty] = CreateDynamicSphere(1371.578, 665.364, 10.890, 2.0, -1, -1, -1);
    EMSStuff[EMSGarage] = CreateDynamicSphere(1362.835, 711.692, 10.920, 2.0, -1, -1, -1);		
    EMSStuff[EMSBrankas] = CreateDynamicSphere(1364.342, 665.669, 10.890, 2.0, -1, -1, -1);
    EMSStuff[EMSLocker] = CreateDynamicSphere(1360.008, 671.306, 10.890, 2.0, -1, -1, -1);
    EMSStuff[EMSDesk] = CreateDynamicSphere(1394.397, 684.772, 10.890, 2.0, -1, -1, -1);
    EMSStuff[EMSItemsLocker] = CreateDynamicSphere(1366.820, 671.430, 10.890, 2.0, -1, -1, -1);
    EMSStuff[EMSHeli] = CreateDynamicSphere(1773.228149, -1122.462402, 38.298686, 2.0, -1, -1, -1);
    EMSStuff[EMSGaragePOS][0] = 1363.044; 
    EMSStuff[EMSGaragePOS][1] = 711.722; 
    EMSStuff[EMSGaragePOS][2] = 10.920;
    
    EMSStuff[EMSSpawnPOS][0] = 1393.090;
    EMSStuff[EMSSpawnPOS][1] = 719.033;
    EMSStuff[EMSSpawnPOS][2] = 10.820;
    EMSStuff[EMSSpawnPOS][3] = 90.687;
}
    ...
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(AccountData[playerid][pFaction] == FACTION_EMS)
        {
            ...
            if(IsPlayerInDynamicArea(playerid, EMSStuff[EMSHeli]) && AccountData[playerid][pDutyEms])
            {
                ...
                if(!EMSSpawnHeli[playerid])
                {
                    EMSHeliVeh[playerid] = AddStaticVehicleEx(487, 1773.228149, -1122.462402, 38.298686, 87.394409, 3, 1, 60000, 1);
            ...
            {
                if(AccountData[playerid][pFactionRank] < 10) return ShowTDN(playerid, NOTIFICATION_ERROR, "Minimal rank Wadir Utama untuk akses store!");
...
hook OnPlayerEnterDynArea(playerid, STREAMER_TAG_AREA:areaid)
{
    if(AccountData[playerid][pFaction] == FACTION_EMS)
    {
        ...
        {
            ShowKey(playerid, "[ALT] Brankas EMS");
        }
        ...
        if(areaid == EMSStuff[EMSHeli] && AccountData[playerid][pDutyEms])
        {
            if(!EMSSpawnHeli[playerid])
            {
                ShowKey(playerid, "[ALT] Spawn Heli");               
            }
            else 
            {
                ShowKey(playerid, "[ALT] Kembalikan Heli");
            }
        }

        if(areaid == EMSStuff[EMSItemsLocker] && AccountData[playerid][pDutyEms])
        {
            ShowKey(playerid, "[ALT] Store EMS");
        }
    }
    return 1;
}
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    switch(dialogid)
    {
        ...
        case DIALOG_EMSBKCONFIRM:
        {
            ...
            switch(AccountData[playerid][pTempSQLFactMemberID])
            {
                case 0:// perban
                {
                    ...
                    mysql_format(mdb_query, sha, sizeof(sha), "UPDATE `stuffs` SET `emsmoneyvault`=%d WHERE id=0", EMSMoneyVault);
                    mysql_tquery(mdb_query, sha);
        ...
        case DIALOG_EMS_PANEL:
        {
            ...
            switch(listitem)
            {
                ...
                {   
                    ...
                    PlayerTextDrawSetString(playerid, ProgressBar[playerid][3], "TREATMENT");
        ...
        case DIALOG_EMSVAULT:
        {
            ...
            switch(listitem)
            {
                ...
                case 1: //withdraw
                {
                    ...
                    format(str, sizeof(str), "Nama Item\tJumlah\tBerat (-/-)\n");
                    mysql_query(mdb_query, "SELECT * FROM `ems_brankas` WHERE `PID`=0");
                    if(cache_num_rows() > 0)
                    {
        ...
        case DIALOG_EMSVAULT_IN:
        {
            ...
            new invstr[1028];
            mysql_format(mdb_query, shstr, sizeof(shstr), "SELECT * FROM `ems_brankas` WHERE `PID`=0 AND `Item` = '%s'", FactionBrankas[playerid][factionBrankasTemp]);
            mysql_query(mdb_query, shstr);
            new rows = cache_num_rows();
            if(rows > 0)
            {
                mysql_format(mdb_query, invstr, sizeof(invstr), "UPDATE `ems_brankas` SET `Quantity` = `Quantity` + %d WHERE `PID` = 0 AND `Item`='%s'", quantity, FactionBrankas[playerid][factionBrankasTemp]);
                mysql_tquery(mdb_query, invstr, "OnEMSDeposit", "i", playerid);
            }
            else 
            {
                mysql_format(mdb_query, invstr, sizeof(invstr), "INSERT INTO `ems_brankas` SET `PID`=0, `Item`='%s', `Model`=%d, `Quantity`=%d", FactionBrankas[playerid][factionBrankasTemp], FactionBrankas[playerid][factionBrankasModel], quantity);
                mysql_tquery(mdb_query, invstr, "OnEMSDeposit", "i", playerid);
            }
            AccountData[playerid][menuShowed] = false;
        }
        ...
        case DIALOG_EMSVAULT_OUT:
        {
            ...
            {
                mysql_format(mdb_query, jts, sizeof(jts), "UPDATE `ems_brankas` SET `Quantity`=%d WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasQuant], FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(mdb_query, jts);
            }
            else 
            {
                mysql_format(mdb_query, jts, sizeof(jts), "DELETE FROM `ems_brankas` WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(mdb_query, jts);
            }
        ...
        case DIALOG_EMSKICKMEMBER:
        {
            ...
            if(cache_num_rows())
            {
                ...
                new strgbg[158];
                mysql_format(mdb_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d AND `PVeh_Faction` = 3", pidrow);
                mysql_tquery(mdb_query, strgbg);
        ...
        case DIALOG_EMS_INVITE:
        {
            ...
            mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Faction` = 3, `Char_FactionRank` = 1 WHERE `pID` = %d", AccountData[targetid][pID]);
            mysql_tquery(mdb_query, icsr);
...
public EMSReviving(playerid, targetid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 10)
    {
        ...
        if(AccountData[targetid][pInjured])
        {
            ...
            forex(i, 24)
            {
                PlayerTextDrawHide(targetid, ATRP_Injured[targetid][i]);
            }
        }
