...
Vehicle_ShowBagasi(playerid, vehid)
{
	...
	mysql_format(mdb_query, fhsrs, sizeof(fhsrs), "SELECT * FROM `vehicle_bagasi` WHERE `vID`=%d", PlayerVehicle[vehid][pVehID]);
	execute = mysql_query(mdb_query, fhsrs, true);
...
stock Vehicle_Delete(vid)
{
	...
	new query[200];
	mysql_format(mdb_query, query, sizeof(query), "DELETE FROM player_vehicles WHERE id = '%d'", PlayerVehicle[vid][pVehID]);
	mysql_tquery(mdb_query, query);

	for (new i = 0; i < MAX_VEHICLE_OBJECT; i ++) if (VehicleObjects[vid][i][vehObjectExists])
	{
		new dbstr[200];
		mysql_format(mdb_query, dbstr, sizeof(dbstr), "DELETE FROM vehicle_object WHERE vehicle = '%d'", PlayerVehicle[vid][pVehID]);
		mysql_tquery(mdb_query, dbstr);
		Vehicle_ObjectDestroy(vid);
	}

	new vstr[188];
	mysql_format(mdb_query, vstr, sizeof(vstr), "DELETE FROM vehicle_bagasi WHERE vID = '%d'", PlayerVehicle[vid][pVehID]);
	mysql_tquery(mdb_query, vstr);
	...
	Iter_SafeRemove(PvtVehicles, vid, vid);
	return 1;
}
...
VehicleImport_Create(ownerid, modelid, factionid, Float:x, Float:y, Float:z, Float:a, color1, color2, cost)
{
	...
	if ((vehicleid = Iter_Free(PvtVehicles)) != cellmin)
	{
		...
		mysql_tquery(mdb_query, "INSERT INTO `player_vehicles` (`PVeh_Faction`) VALUES ('0')", "OnVehCreated", "i", vehicleid);
		return vehicleid;
	}
	return -1;
}
...
Vehicle_Create(ownerid, modelid, factionid, Float:x, Float:y, Float:z, Float:a, color1, color2, cost)
{
	...
	if ((vehicleid = Iter_Free(PvtVehicles)) != cellmin)
	{
		...
		// mysql_format(mdb_query, cQuery, sizeof(cQuery), "INSERT INTO `player_vehicles` (`PVeh_OwnerID`, `PVeh_ModelID`, `PVeh_Faction`, `PVeh_Price`, `PVeh_Health`, `PVeh_Fuel`, `PVeh_Parked`, `PVeh_Housed`, `PVeh_Families`, `PVeh_FactionGarage`, `PVeh_Rental`, `PVeh_RentTime`, `PVeh_Plate`, \
		...
		mysql_tquery(mdb_query, "INSERT INTO `player_vehicles` (`PVeh_Faction`) VALUES('0')", "OnVehCreated", "d", vehicleid);
		return vehicleid;
	}
	return -1;
}
...
VehicleFaction_Create(ownerid, modelid, factionid, Float:x, Float:y, Float:z, Float:a, color1, color2, cost)
{
	...
	if ((vehicleid = Iter_Free(PvtVehicles)) != cellmin)
	{
		...
		mysql_tquery(mdb_query, sprintf("INSERT INTO `player_vehicles` (`PVeh_Faction`) VALUES(%d)", PlayerVehicle[vehicleid][pVehFaction]), "OnVehFactionCreated", "ii", ownerid, vehicleid);
		return vehicleid;
	}
	return -1;
}
...
public OnLoadVehicleMod(vehid)
{
	if(IsValidVehicle(PlayerVehicle[vehid][pVehPhysic]))
	{
		...
		if(PlayerVehicle[vehid][pVehFaction] == FACTION_POLISI)
		{
			...
			{
				AddVehicleComponent(PlayerVehicle[vehid][pVehPhysic], 1010);
				AddVehicleComponent(PlayerVehicle[vehid][pVehPhysic], 1025);	
...
public UnloadRentalVehicle(carid)
{
	...
	if(IsValidVehicle(carid))
	{
		if(PlayerVehicle[carid][pVehRental] != -1 || PlayerVehicle[carid][pVehRentTime] > 0)
		{
			...
			new strgbg[256];
			mysql_format(mdb_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `id` = %d", PlayerVehicle[carid][pVehID]);
			mysql_tquery(mdb_query, strgbg);

			Iter_SafeRemove(PvtVehicles, carid, carid);
		}
	}
	return 1;
}
...
Function: Vehicle_Load(playerid)
{
	...
	for (new i = 0; i != cache_num_rows(); i ++)
	{
		if ((vehicleid = Iter_Free(PvtVehicles)) != cellmin)
		{
			...
			new vQuery[300];
			mysql_format(mdb_query, vQuery, sizeof(vQuery), "SELECT * FROM `vehicle_object` WHERE `vehicle` = '%d' ORDER BY `id` ASC LIMIT %d", PlayerVehicle[vehicleid][pVehID], MAX_VEHICLE_OBJECT);
			mysql_tquery(mdb_query, vQuery, "Vehicle_ObjectLoad", "d", vehicleid);
...
Function:LoadPlayerVehicle(playerid)
{
	...
	mysql_format(mdb_query, query, sizeof(query), "SELECT * FROM `player_vehicles` WHERE `PVeh_OwnerID` = '%d' ORDER BY `id` ASC", AccountData[playerid][pID]);
	mysql_query(mdb_query, query, true);
	...
	if(count > 0)
	{
		for(new z = 0; z < count; z ++)
		{
			...
			new vQuery[300];
			mysql_format(mdb_query, vQuery, sizeof(vQuery), "SELECT * FROM `vehicle_object` WHERE `vehicle` = '%d' ORDER BY `id` ASC LIMIT %d", PlayerVehicle[i][pVehID], MAX_VEHICLE_OBJECT);
			mysql_tquery(mdb_query, vQuery, "Vehicle_ObjectLoad", "d", i);
...
stock SavePlayerVehicle(i)
{
	...
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "UPDATE `player_vehicles` SET ");
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_ModelID`=%d, ", cQuery, PlayerVehicle[i][pVehModelID]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_OwnerID`=%d, ", cQuery, PlayerVehicle[i][pVehOwnerID]);
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Price`=%d, ", cQuery, PlayerVehicle[i][pVehPrice]);
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Parked`=%d, ", cQuery, PlayerVehicle[i][pVehParked]);
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Housed`=%d, ", cQuery, PlayerVehicle[i][pVehHouseGarage]);
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Helipad`=%d, ", cQuery, PlayerVehicle[i][pVehHelipadGarage]);
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Families`=%d, ", cQuery, PlayerVehicle[i][pVehFamiliesGarage]);
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_FactionGarage`=%d, ", cQuery, PlayerVehicle[i][pVehFactStored]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Faction`=%d, ", cQuery, PlayerVehicle[i][pVehFaction]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_RentTime`=%d, ", cQuery, PlayerVehicle[i][pVehRentTime]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Rental`=%d, ", cQuery, PlayerVehicle[i][pVehRental]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Plate`='%e', ", cQuery, PlayerVehicle[i][pVehPlate]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_PlateTime`=%d, ", cQuery, PlayerVehicle[i][pVehPlateTime]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_PlateOwned`=%d, ", cQuery, PlayerVehicle[i][pVehPlateOwn]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Health`='%f', ", cQuery, PlayerVehicle[i][pVehHealth]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Fuel`=%d, ", cQuery, PlayerVehicle[i][pVehFuel]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Locked`=%d, ", cQuery, PlayerVehicle[i][pVehLocked]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod0`=%d, ", cQuery, PlayerVehicle[i][pVehMod][0]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod1`=%d, ", cQuery, PlayerVehicle[i][pVehMod][1]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod2`=%d, ", cQuery, PlayerVehicle[i][pVehMod][2]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod3`=%d, ", cQuery, PlayerVehicle[i][pVehMod][3]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod4`=%d, ", cQuery, PlayerVehicle[i][pVehMod][4]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod5`=%d, ", cQuery, PlayerVehicle[i][pVehMod][5]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod6`=%d, ", cQuery, PlayerVehicle[i][pVehMod][6]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod7`=%d, ", cQuery, PlayerVehicle[i][pVehMod][7]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod8`=%d, ", cQuery, PlayerVehicle[i][pVehMod][8]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod9`=%d, ", cQuery, PlayerVehicle[i][pVehMod][9]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod10`=%d, ", cQuery, PlayerVehicle[i][pVehMod][10]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod11`=%d, ", cQuery, PlayerVehicle[i][pVehMod][11]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod12`=%d, ", cQuery, PlayerVehicle[i][pVehMod][12]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod13`=%d, ", cQuery, PlayerVehicle[i][pVehMod][13]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod14`=%d, ", cQuery, PlayerVehicle[i][pVehMod][14]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod15`=%d, ", cQuery, PlayerVehicle[i][pVehMod][15]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Mod16`=%d, ", cQuery, PlayerVehicle[i][pVehMod][16]);

	/* Vehicle Upgrade */
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_EngineUpgrade`=%d, ", cQuery, PlayerVehicle[i][pVehEngineUpgrade]);
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_BodyUpgrade`=%d, ", cQuery, PlayerVehicle[i][pVehBodyUpgrade]);
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_BodyRepair`=%.3f, ", cQuery, PlayerVehicle[i][pVehBodyRepair]);

	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Weapon1`=%d, ", cQuery, PlayerVehicle[i][pVehWeapon][0]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Weapon2`=%d, ", cQuery, PlayerVehicle[i][pVehWeapon][1]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Weapon3`=%d, ", cQuery, PlayerVehicle[i][pVehWeapon][2]);
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Ammo1`=%d, ", cQuery, PlayerVehicle[i][pVehAmmo][0]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Ammo2`=%d, ", cQuery, PlayerVehicle[i][pVehAmmo][1]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Ammo3`=%d, ", cQuery, PlayerVehicle[i][pVehAmmo][2]);
    
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Damage0`=%d, ", cQuery, PlayerVehicle[i][pVehDamage][0]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Damage1`=%d, ", cQuery, PlayerVehicle[i][pVehDamage][1]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Damage2`=%d, ", cQuery, PlayerVehicle[i][pVehDamage][2]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Damage3`=%d, ", cQuery, PlayerVehicle[i][pVehDamage][3]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_PosX`='%f', ", cQuery, PlayerVehicle[i][pVehPos][0]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_PosY`='%f', ", cQuery, PlayerVehicle[i][pVehPos][1]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_PosZ`='%f', ", cQuery, PlayerVehicle[i][pVehPos][2]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_PosA`='%f', ", cQuery, PlayerVehicle[i][pVehPos][3]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Neon`=%d, ", cQuery, PlayerVehicle[i][pVehNeon]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Paintjob`=%d, ", cQuery, PlayerVehicle[i][pVehPaintjob]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Color1`=%d, ", cQuery, PlayerVehicle[i][pVehColor1]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Color2`=%d, ", cQuery, PlayerVehicle[i][pVehColor2]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_World`=%d, ", cQuery, PlayerVehicle[i][pVehWorld]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Interior`=%d, ", cQuery, PlayerVehicle[i][pVehInterior]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Impounded`=%d, ", cQuery, PlayerVehicle[i][pVehImpounded]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_ImpoundedTime`=%d, ", cQuery, PlayerVehicle[i][pVehImpoundDuration]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_ImpoundedPrice`=%d, ", cQuery, PlayerVehicle[i][pVehImpoundFee]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_ImpoundedReason`='%s', ", cQuery, PlayerVehicle[i][pVehImpoundReason]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Insuranced`=%d, ", cQuery, PlayerVehicle[i][pVehInsuranced]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Donation`=%d, ", cQuery, PlayerVehicle[i][vehDonation]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_DCTime`=%d, ", cQuery, gettime() + 3600);
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Capacity`=%f ", cQuery, PlayerVehicle[i][pVehCapacity]);
    mysql_format(mdb_query, cQuery, sizeof(cQuery), "%sWHERE `id` = '%d'", cQuery, PlayerVehicle[i][pVehID]);
    mysql_query(mdb_query, cQuery, true);
	return 1;
}
...
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_PosX`='%f', ", cQuery, PlayerVehicle[i][pVehPos][0]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_PosY`='%f', ", cQuery, PlayerVehicle[i][pVehPos][1]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_PosZ`='%f', ", cQuery, PlayerVehicle[i][pVehPos][2]+0.1);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_PosA`='%f', ", cQuery, PlayerVehicle[i][pVehPos][3]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Health`='%f', ", cQuery, PlayerVehicle[i][pVehHealth]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Fuel`=%d, ", cQuery, PlayerVehicle[i][pVehFuel]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Interior`=%d, ", cQuery, PlayerVehicle[i][pVehInterior]);
...
// 				mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`%s`='%d', ", cQuery, tempString, PlayerVehicle[i][pVehMod][z]);
// 			}
...
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_RentTime`='%d', ", cQuery, PlayerVehicle[i][pVehRentTime]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Parked`='%d', ", cQuery, PlayerVehicle[i][pVehParked]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Broken`='%d', ", cQuery, PlayerVehicle[i][pVehBroken]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Housed`='%d', ", cQuery, PlayerVehicle[i][pVehHouseGarage]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Helipad`='%d', ", cQuery, PlayerVehicle[i][pVehHelipadGarage]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_Families`='%d', ", cQuery, PlayerVehicle[i][pVehFamiliesGarage]);
// 			mysql_format(mdb_query, cQuery, sizeof(cQuery), "%s`PVeh_FactionGarage`='%d', ", cQuery, PlayerVehicle[i][pVehFactStored]);
...
// 				mysql_format(mdb_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `id` = %d", PlayerVehicle[i][pVehID]);
// 				mysql_tquery(mdb_query, strgbg);
// 			}*/
	...
	if(IsValidVehicle(vehicleid))
	{
		...
		if(id != -1)
		{
			...
			if(PlayerVehicle[id][pVehRental] != -1 || PlayerVehicle[id][pVehRentTime] > 0)
			{
				...
				mysql_format(mdb_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `id` = %d", PlayerVehicle[id][pVehID]);
				mysql_tquery(mdb_query, strgbg);

				Iter_SafeRemove(PvtVehicles, id, id);
			}
			else 
			{
...
{
	new tmpcount;
	if(hslot < 4 && hslot > MAX_PLAYER_VEHICLE) return -1;
