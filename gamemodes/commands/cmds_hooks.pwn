...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_BAGASI)
    {
        ...
        switch(listitem)
        {
            case 0: //deposit
            {
                ...
                execute = mysql_query(mdb_query, tss, true);
    ...
    else if(dialogid == DIALOG_BAGASI_IN)
    {
        ...
        mysql_format(mdb_query, shstr, sizeof(shstr), "SELECT * FROM `vehicle_bagasi` WHERE `vID`=%d AND `Item`='%s'", PlayerVehicle[vehid][pVehID], VehicleBagasi[playerid][vehiclebagasiTemp]);
        execute = mysql_query(mdb_query, shstr, true);
        new rows = cache_num_rows();
        if(rows > 0)
        {
            mysql_format(mdb_query, invstr, sizeof(invstr), "UPDATE `vehicle_bagasi` SET `Quantity` = `Quantity` + %d WHERE `vID` = %d AND `Item` = '%s'", quantity, PlayerVehicle[vehid][pVehID], VehicleBagasi[playerid][vehiclebagasiTemp]);
            mysql_tquery(mdb_query, invstr, "OnBagasiDeposit", "i", playerid);
        }
        else 
        {
            mysql_format(mdb_query, invstr, sizeof(invstr), "INSERT INTO `vehicle_bagasi` SET `vID`=%d, `Item`='%s', `Model`=%d, `Quantity`=%d", PlayerVehicle[vehid][pVehID], VehicleBagasi[playerid][vehiclebagasiTemp], VehicleBagasi[playerid][vehiclebagasiModel], quantity);
            mysql_tquery(mdb_query, invstr, "OnBagasiDeposit", "i", playerid);
        }
    ...
    else if(dialogid == DIALOG_BAGASI_WITHDRAW)
    {
        ...
        new tss[100];
        mysql_format(mdb_query, tss, sizeof(tss), "SELECT * FROM `vehicle_bagasi` WHERE `vID`=%d", PlayerVehicle[vehid][pVehID]);
    ...
    else if(dialogid == DIALOG_BAGASI_OUT)
    {
        ...
        {
            mysql_format(mdb_query, jts, sizeof(jts), "UPDATE `vehicle_bagasi` SET `Quantity`=%d WHERE `ID`=%d", VehicleBagasi[playerid][vehiclebagasiQuant], VehicleBagasi[playerid][vehiclebagasiID]);
            mysql_tquery(mdb_query, jts);
        }
        else 
        {
            mysql_format(mdb_query, jts, sizeof(jts), "DELETE FROM `vehicle_bagasi` WHERE `ID`=%d", VehicleBagasi[playerid][vehiclebagasiID]);
            mysql_tquery(mdb_query, jts);
        }
        SwitchVehicleBoot(PlayerVehicle[vehid][pVehPhysic], false);
    ...
    else if(dialogid == DIALOG_CHANGE_PASSWORD)
    {
        ...
        new query[255];
        mysql_format(mdb_query, query, sizeof(query), "UPDATE `playerucp` SET `password`='%s', `salt`='%e' WHERE `ucp`='%s'", AccountData[playerid][pPassword], AccountData[playerid][pSalt], AccountData[playerid][pUCP]);
...
AddCapacityVehicle(vehicleid, const item[], quantity)
{
    ...
    {
        PlayerVehicle[vehicleid][pVehCapacity] += quantity*0.090;
    }
    ...
    new sctr[596];
    mysql_format(mdb_query, sctr, sizeof(sctr), "UPDATE `player_vehicles` SET `PVeh_Capacity`=%f WHERE `id`=%d", PlayerVehicle[vehicleid][pVehCapacity], PlayerVehicle[vehicleid][pVehID]);
