...
CMD:entercar(playerid, params[])
{
    ...
    if(!AccountData[playerid][pAdminDuty])
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak bisa menggunakan perintah admin saat tidak bertugas");
...
CMD:checkdiscorducp(playerid, params[])
{
	...
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "SELECT `ucp` FROM `playerucp` WHERE `DiscordID`='%s'", DCID);
	execute = mysql_query(mdb_query, cQuery, true);
	if(cache_num_rows())
	{
		new playerUCP[24];
...
CMD:setthestars(playerid, params[])
{
	...
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "UPDATE `player_characters` SET `Char_TheStars`=%d, `Char_TheStarsTime`=%d WHERE `pID`=%d", AccountData[otherid][pTheStars], AccountData[otherid][pTheStarsTime], AccountData[otherid][pID]);
...
CMD:osetstars(playerid, params[])
{
	...
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "UPDATE `player_characters` SET `Char_TheStars`=1, `Char_TheStarsTime`=%d WHERE `Char_Name`='%s'", duration, playerName);
	mysql_tquery(mdb_query, cQuery);
	return 1;
}
...
CMD:makewargaoff(playerid, params[]) 
{
	...
	new query[255];
	mysql_format(mdb_query, query, sizeof(query), "SELECT `pID` FROM `player_characters` WHERE `Char_Name`='%e'", playerName);
	mysql_tquery(mdb_query, query, "MakeWargaOffline", "ds", playerid, playerName);
	return 1;
}
...
CMD:resetdelay(playerid, params[])
{
	...
	if(!strcmp(type, "mower", true))
	{
		...
		mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_MowerTime`=0 WHERE `pID`=%d", AccountData[otherid][pID]);
	...
	else if(!strcmp(type, "delivery", true))
	{
		...
		mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_DeliveryDelay`=0 WHERE `pID`=%d", AccountData[otherid][pID]);
		mysql_tquery(mdb_query, icsr);
	}
	else if(!strcmp(type, "sweeper", true))
	{
		...
		mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_SweeperDelay`=0 WHERE `pID`=%d", AccountData[otherid][pID]);
		mysql_tquery(mdb_query, icsr);
	}
	else if(!strcmp(type, "forklift", true))
	{
		...
		mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_ForkliftDelay`=0 WHERE `pID`=%d", AccountData[otherid][pID]);
		mysql_tquery(mdb_query, icsr);
	}
...
CMD:customnumber(playerid, params[])
{
	...
	new query[598];
	mysql_format(mdb_query, query, sizeof(query), "SELECT Char_PhoneNum FROM player_characters WHERE Char_PhoneNum='%s'", number);
	mysql_tquery(mdb_query, query, "PhoneNumberCustom", "iis", playerid, otherid, number);
	return 1;
}
...
CMD:customrek(playerid, params[])
{
	...
	new icsr[255];
	mysql_format(mdb_query, icsr, sizeof(icsr), "SELECT `Char_BankRek` FROM `player_characters` WHERE `Char_BankRek`=%d", newrek);
	mysql_tquery(mdb_query, icsr, "RekeningCustom", "iid", playerid, otherid, newrek);
	return 1;
}
...
FUNC::RekeningCustom(playerid, otherid, rekening)
{
	...
	else
	{
		...
		new query[255];
		mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_BankRek`= %d WHERE `pID`=%d", rekening, AccountData[otherid][pID]);
...
public PhoneNumberCustom(playerid, otherid, const number[])
{
	...
	else
	{
		...
		new query[255];
		mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_PhoneNum`='%s' WHERE `pID`=%d", number, AccountData[otherid][pID]);
		mysql_tquery(mdb_query, query);
	}
	return 1;
}
...
CMD:changegender(playerid, params[])
{
	...
	new icsr[200];
	mysql_format(mdb_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Gender`=%d WHERE `pID`=%d", gender, AccountData[id][pID]);
	mysql_tquery(mdb_query, icsr);
	return 1;
}
...
CMD:checkmask(playerid, params[])
{
	...
	execute = mysql_query(mdb_query, sprintf("SELECT `Char_Name` FROM `player_characters` WHERE `Char_MaskID` = '%d';", strval(params)));
	if(cache_num_rows()) {
...
CMD:makewarga(playerid, params[])
{
	...
	new query[200];
	mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Faction`=%d, `Char_FactionRank`=%d, `Char_Family`=%d, `Char_FamilyRank`=%d WHERE `pID`=%d", AccountData[otherid][pFaction], AccountData[otherid][pFactionRank], AccountData[otherid][pFamily], AccountData[otherid][pFamilyRank], AccountData[otherid][pID]);
	mysql_tquery(mdb_query, query);
...
{
	if(AccountData[playerid][pAdmin] < 6 || AccountData[playerid][pAdminBS] < 1) return PermissionError(playerid);
...
CMD:askell(playerid, params[])
{
	...
	if(sscanf(params, "d", times))
		return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/askell [seconds]");
...
CMD:arelease(playerid, params[])
{
	if(AccountData[playerid][pAdmin] < 4)
        return ShowTDN(playerid, NOTIFICATION_ERROR, "Kamu harus menjadi Admin level 4.");
...
CMD:sendto(playerid, params[])
{
	...
	{
		Syntax(playerid, "/sendto [name/playerid] (ls, lv, sf, asuransi, carnaval, bandara, rs, balkot, pedagang, bengkel, polisi)");
		return 1;
	}
	...
	else if(!strcmp(option, "asuransi", true))
	{
		if(GetPlayerState(otherid) == PLAYER_STATE_DRIVER) 
		{
            SetVehiclePos(GetPlayerVehicleID(otherid),1425.252563, 1058.143066, 10.517755);
        }
        else 
		{
            SetPlayerPosition(otherid, 1425.252563, 1058.143066, 10.517755, 350);
        }
        SetPlayerFacingAngle(otherid,91.141853);
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
	...
	else if(!strcmp(option, "bandara", true))
	{
		if(GetPlayerState(otherid) == PLAYER_STATE_DRIVER) 
		{
            SetVehiclePos(GetPlayerVehicleID(otherid),1697.543457, -2259.316162, 13.535642);
        }
        else 
		{
            SetPlayerPositionEx(otherid, 1697.543457, -2259.316162, 13.535642, 2000);
        }
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
	...
	else if(!strcmp(option, "rs", true))
	{
		if(GetPlayerState(otherid) == PLAYER_STATE_DRIVER) 
		{
            SetVehiclePos(GetPlayerVehicleID(otherid),1377.666, 719.911, 10.820);
        }
        else 
		{
            SetPlayerPositionEx(otherid, 1377.666, 719.911, 10.820, 174.570465, 2000);
        }
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
	...
	else if(!strcmp(option, "pedagang", true))
	{
		if(GetPlayerState(otherid) == PLAYER_STATE_DRIVER) 
		{
            SetVehiclePos(GetPlayerVehicleID(otherid),656.798156, -1854.403564, 6.115148);
        }
        else 
		{
            SetPlayerPositionEx(otherid, 656.798156, -1854.403564, 6.115148, 354.161468, 2000);
        }
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Pedagang.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Pedagang.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "bengkel", true))
	{
		if(GetPlayerState(otherid) == PLAYER_STATE_DRIVER) 
		{
            SetVehiclePos(GetPlayerVehicleID(otherid),-64.748939, 1044.376708, 19.593750);
        }
        else 
		{
            SetPlayerPositionEx(otherid, -64.748939, 1044.376708, 19.593750, 2000);
        }
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Bengkel.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Bengkel.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "polisi", true))
	{
		if(GetPlayerState(otherid) == PLAYER_STATE_DRIVER) 
		{
            SetVehiclePos(GetPlayerVehicleID(otherid),957.726196, 2412.326416, 10.854869);
        }
        else 
		{
            SetPlayerPositionEx(otherid, 957.726196, 2412.326416, 10.854869, 203.620666, 2000);
        }
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Kanpol.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Kanpol", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
...
CMD:p2p(playerid, params[])
{
 ...
 new otherid;
 new string[300];
 if(sscanf(params, "uu", otherid, userid))
        return ax(playerid, "/p2p [playerid/PartOfName] to [playerid/PartOfName]");

    if(!IsPlayerConnected(otherid))
        return Syntax(playerid, "Player not connected!");

  if(!IsPlayerConnected(userid))
        return Syntax(playerid, "Player not connected!");
  
	SendPlayerToPlayer(otherid, userid);
 	SendStaffMessage(playerid, ""WHITE_E"You've teleported "YELLOW_E"%s "WHITE_E"to "YELLOW_E"%s "WHITE_E"position", AccountData[otherid][pName], AccountData[userid][pName]);
 	SendClientMessageEx(otherid, "{FF0000}%s "WHITE_E"has teleported "YELLOW_E"%s "WHITE_E"to your position", AccountData[playerid][pName], AccountData[userid][pName]);
 	SendClientMessageEx(userid, "{FF0000}%s"WHITE_E" has telerpoted you to "YELLOW_E"%s "WHITE_E"position", AccountData[playerid][pname], AccountData[otherid][pname]);
 return 1;
}
*/
...
CMD:ostats(playerid, params[])
{
	...
	strcat(shstr, "pID, Char_UCP, Char_Name, Char_Age, Char_BodyHeight, Char_BodyWeight, Char_Origin, Char_Gender, marriedto, Char_Story, Char_Job, Char_PhoneNum, Char_Faction, Char_FactionRank, Char_Family, Char_FamilyRank, Char_RedMoney, Char_Money, Char_BankMoney, Char_Health, Char_Armour, Char_Hunger, Char_Thirst, Char_Stress, Char_Warn, Char_Level, Char_LevelUp, Char_Admin, Char_TheStars, Char_TheStarsTime, Char_Vip, Char_VipTime, Char_TogPM, Char_Skin, Char_RegisterDate, Char_LastLogin");
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "SELECT %s FROM player_characters WHERE Char_Name='%e' LIMIT 1;", shstr, playerName);
	mysql_tquery(mdb_query, cQuery, "OfflineStats", "ds", playerid, playerName);
	return 1;
}

CMD:setadminbs(playerid, params[])
{
    if (AccountData[playerid][pAdmin] < 6) 
        return PermissionError(playerid);

    new otherid, level;
    if (sscanf(params, "ii", otherid, level))
        return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/setadminbs [id] [0/1]");

    // Validasi ID
    if (otherid < 0 || otherid >= MAX_PLAYERS || !IsPlayerConnected(otherid))
        return ShowTDN(playerid, NOTIFICATION_ERROR, "Player ID tidak valid atau tidak terkoneksi!");

    // Cek apakah target adalah admin
    if (AccountData[otherid][pAdmin] < 1)
        return ShowTDN(playerid, NOTIFICATION_ERROR, "Target bukan admin, tidak bisa diatur.");

    // Validasi level
    if (level != 0 && level != 1)
        return ShowTDN(playerid, NOTIFICATION_ERROR, "Level hanya bisa 0 atau 1!");

    // Set akses
    AccountData[otherid][pAdminBS] = level;
    ...
    mysql_format(mdb_query, query, sizeof(query), 
        "UPDATE `player_characters` SET `Char_AdminBS` = %d WHERE `Char_Name` = '%s' LIMIT 1",
        level, name);
    mysql_tquery(mdb_query, query);

    // Feedback ke admin
    new msg[144];
    format(msg, sizeof(msg), "[ADMIN] Admin %s telah diatur ke level %d untuk handle badside.", name, level);
    SendClientMessage(playerid, -1, msg);

    // Feedback ke target
    if (level == 0)
        SendClientMessage(otherid, -1, "[ADMIN] Status Handle Badside Anda telah dihapus.");
    else {
        format(msg, sizeof(msg), "[ADMIN] Anda sekarang memiliki akses Handle Badside level %d.", level);
        SendClientMessage(otherid, -1, msg);
    }
...
CMD:aka(playerid, params[])
{
	...
	GetPlayerIp(otherid, PlayerIP, sizeof(PlayerIP));
	mysql_format(mdb_query, query, sizeof(query), "SELECT Char_Name FROM player_characters WHERE Char_IP='%s'", PlayerIP);
	mysql_tquery(mdb_query, query, "CheckPlayerIP", "is", playerid, PlayerIP);
	return true;
}
...
CMD:akaip(playerid, params[])
{
	...
	mysql_format(mdb_query, query, sizeof(query), "SELECT Char_Name FROM player_characters WHERE IP='%s'", params);
	mysql_tquery(mdb_query, query, "CheckPlayerIP2", "is", playerid, params);
	return true;
}
...
CMD:setcs(playerid, params[])
{
    if(AccountData[playerid][pAdmin] < 2)
 	...
 	Info(otherid, "Your CS has approved by %s.", AccountData[playerid][pAdminname]);
 	
 	if(sscanf(params, "d", otherid)) return Syntax(playerid, "/setcs [playerid]");

 	if(AccountData[otherid][pCharStory] != 0) return Error(playerid, "Character story anda sudah di approve!");
 	SendStaffMessage(COLOR_RED, "AdmCmd: %s has approved Character Story %s (ID: %d).", AccountData[playerid][pAdminname], ReturnName(otherid), otherid);
 	SendClientMessageEx(otherid, COLOR_RED, "Your CS has approved by %s", AccountData[playerid][pAdminname]);
 	AccountData[otherid][pCharStory] = 1;
 	return 1;
}
...
CMD:unsetcs(playerid, params[])
{
    if(AccountData[playerid][pAdmin] < 2)
 	...
 	if(sscanf(params, "d", otherid)) return Syntax(playerid, "/unsetcs [playerid]");
 	if(AccountData[otherid][pCharStory] != 1) return Error(playerid, "Character story anda sudah di approve!");
 	SendStaffMessage(COLOR_RED, "AdmCmd: %s has un approved CS %s (ID: %d).", AccountData[playerid][pAdminname], ReturnName(otherid), otherid);
 	AccountData[otherid][pCharStory] = 0;
 	return 1;
}
...
CMD:osetcs(playerid, params[])
{
	...
	{
	    Syntax(playerid, "/osetcs <player name>");
 		return 1;
 	}
    ...
    new query[512];
	mysql_format(mdb_query, query, sizeof(query), "SELECT pID FROM player_characters WHERE Char_Name='%e'", name);
	mysql_tquery(mdb_query, query, "SetCSPlayer", "is", playerid, name);
 	return true;
}
...
function SetCSPlayer(adminid, NameToSetcs[])
{
	...
	else
	{
		...
		SendStaffMessage(COLOR_RED, "AdmCmd: "GREY2_E"Admin %s telah approved Char Story player %s", AccountData[adminid][pAdminname], NameToSetcs);
		new query[512];
		mysql_format(mdb_query, query, sizeof(query), "UPDATE player_characters SET Char_Story=%d, WHERE pID=%d", setcsbyanang, RegID);
		mysql_tquery(mdb_query, query);
	}
	return 1;
}
...
CMD:ounsetcs(playerid, params[])
{
    ...
    new query[512];
	mysql_format(mdb_query, query, sizeof(query), "SELECT reg_id FROM players WHERE username='%e'", name);
	mysql_tquery(mdb_query, query, "UnsetCSPlayer", "is", playerid, name);
 	return true;
}
...
CMD:togooc(playerid, params[])
{
    ...
    if(TogOOC == 0)
    {
        SendClientMessageToAllEx(TOMATO, "AdmCmd: [%s] mematikan chat global ooc.", AccountData[playerid][pAdminname]);
        TogOOC = 1;
    }
    else
    {
        SendClientMessageToAllEx(TOMATO, "AdmCmd: [%s] menghidupkan chat ooc global (DON'T SPAM).", AccountData[playerid][pAdminname]);
        TogOOC = 0;
    }
    return 1;
}*/
...
CMD:owarn(playerid, params[]) 
{
	...
	new query[255];
	mysql_format(mdb_query, query, sizeof(query), "SELECT `pID`, `Char_Warn` FROM `player_characters` WHERE `Char_Name`='%e'", player);
	mysql_tquery(mdb_query, query, "OfflineWarnPlayer", "iss", playerid, player, reason);
	return 1;
}
...
public OfflineWarnPlayer(playerid, name[], reason[])
{
	...
	else 
	{
		...
		new query[255];
		mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Warn`=%d WHERE `pID`=%d", Warn + 1, RegID);
		mysql_tquery(mdb_query, query);

		mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=1, `WarnSender`='%s', `WarnTime`=%d, `WarnReason`='%s'", RegID, AccountData[playerid][pAdminname], gettime(), reason);
		mysql_tquery(mdb_query, query);
	}
	return 1;
}
...
CMD:oreco(playerid, params[])
{
	...
	new query[512];
	mysql_format(mdb_query, query, sizeof(query), "SELECT pID FROM player_characters WHERE Char_Name='%e'", player);
	mysql_tquery(mdb_query, query, "ORecoPlayer", "iss", playerid, player, reason);
...
CMD:ojail(playerid, params[])
{
	...
	new query[512];
	mysql_format(mdb_query, query, sizeof(query), "SELECT `pID` FROM `player_characters` WHERE `Char_Name`='%e'", player);
	mysql_tquery(mdb_query, query, "OfflineJailed", "issi", playerid, player, reason, datez);
	return 1;
}
...
public OfflineJailed(adminid, const NameJail[], const Reason[], time)
{
	...
	else 
	{
		...
		date = gettime();
		mysql_format(mdb_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Jail`=1, `Char_JailTime`=%d, `Char_JailReason`='%s' WHERE `pID`=%d", Jailtime, Reason, RegID);
		mysql_tquery(mdb_query, query);

		mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=2, `WarnTime`=%d, `WarnSender`='%s', `WarnReason`='%s'", RegID, date, GetAdminName(adminid), Reason);
		mysql_tquery(mdb_query, query);
...
public ORecoPlayer(adminid, NameToReco[], RecoReason[])
{
	...
	else 
	{
		...
		mysql_tquery(mdb_query, sprintf("UPDATE player_characters SET Char_BankMoney=0,Char_Money=0 WHERE pID='%d'", RegID));
	}
	return 1;
}
...
CMD:jail(playerid, params[])
{
	...
	date = gettime();
	mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=2, `WarnTime`=%d, `WarnSender`='%s', `WarnReason`='%s'", AccountData[otherid][pID], date, GetAdminName(playerid), reason);
	mysql_tquery(mdb_query, query);
...
CMD:unjail(playerid, params[])
{
	...
	new cQuery[178];
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "DELETE FROM `warninglogs` WHERE `WarnType` = 2 AND `pID` = '%d'", AccountData[otherid][pID]);
	mysql_query(mdb_query, cQuery, false);
	return 1;
}
/*
CMD:kick(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "~> Alasan: %s", reason);
	//SendAdminMessage(COLOR_GREY,"[AdmCmd]"YELLOW" %s{C6E2FF} telah ditendang oleh %s alasan: %s.", ReturnName(userid), AccountData[playerid][pUsername], reason);
	KickEx(userid);

	for(new i = 0;i < 13; i++){
	TextDrawShowForPlayer(userid, KickZ2[i]);
	}

	new str[128];
	format(str, sizeof(str), "Disconnected By Server: You were kicked By : %s", GetName(userid));
	TextDrawSetString(Text:KickZ2[4], str);
	TextDrawShowForPlayer(userid, KickZ2[4]);

	new sstr[128];
	format(sstr, sizeof(sstr), "Reason: %s", reason);
	...
	SetTimerEx("KickTimer", 10000, false, "d", userid);
	return 1;
}*/
CMD:kick(playerid, params[])
{
...
CMD:blockucp(playerid, params[])
{
	...
	new query[248];
	mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `player_bans` (`name`, `ip`, `admin`, `reason`, `ban_date`, `ban_expire`) VALUES ('%s', ' ', '%s', '%s', '%d', '%d')", username, AccountData[playerid][pAdminname], reason, gettime(), banTime);
	mysql_tquery(mdb_query, query);
	return 1;
}
...
CMD:ban(playerid, params[])
{
	...
	new query[248], date;
	date = gettime();
	mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `player_bans` (`name`, `ip`, `admin`, `reason`, `ban_date`, `ban_expire`) VALUES ('%s', '%s', '%s', '%s', '%d', '%d')", AccountData[otherid][pName], PlayerIP, AccountData[playerid][pAdminname], reason, gettime(), banTime);
	mysql_tquery(mdb_query, query);

	mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnTime`=%d, `WarnType`=3, `WarnSender`='%s', `WarnReason`='%s'", AccountData[otherid][pID], date, AccountData[playerid][pAdminname], reason);
	mysql_tquery(mdb_query, query);
...
CMD:unban(playerid, params[])
{
	...
	new query[128];
	mysql_format(mdb_query, query, sizeof(query), "SELECT name,ip FROM player_bans WHERE name = '%s'", tmp);
	mysql_tquery(mdb_query, query, "OnUnbanQueryData", "is", playerid, tmp);
	return 1;
}
...
public OnUnbanQueryData(adminid, BannedName[])
{
	...
	{
		new query[128];
		mysql_format(mdb_query, query, sizeof(query), "DELETE FROM `player_bans` WHERE `name` = '%s'", BannedName);
		mysql_tquery(mdb_query, query);
...
/*CMD:unbanucp(playerid, params[])
{
	...
	new query[128];
	mysql_format(mdb_query, query, sizeof(query), "SELECT name FROM player_bans WHERE name = '%e'", tmp);
	mysql_tquery(mdb_query, query, "UnbanUCP", "is", playerid, tmp);
	return 1;
}
...
CMD:awarn(playerid, params[])
{
	...
	if(!IsPlayerConnected(otherid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Pemain tersebut tidak terkoneksi ke server!");
	mysql_format(mdb_query, dbsr, sizeof(dbsr), "SELECT * FROM `warninglogs` WHERE `pID`=%d", AccountData[otherid][pID]);
	new Cache:execute = mysql_query(mdb_query, dbsr, true);
	if(cache_num_rows())
	{
...
CMD:warn(playerid, params[])
{
	...
	currentTime = gettime();
	mysql_format(mdb_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=1, `WarnSender`='%s', `WarnTime`=%d, `WarnReason`='%s'", AccountData[otherid][pID], GetAdminName(playerid), currentTime, reason);
	mysql_tquery(mdb_query, query);
	return 1;
}
...
CMD:annp(playerid, params[])
{
	...
	if(isnull(params))
    {
	    SendClientMessage(playerid, -1, "/announce <msg>");
	    return 1;
	}
   	if(strfind(params, "~x~", true) != -1)
		return SendClientMessage(playerid, -1, "ERROR: ~x~ is not allowed in announce.");
	if(strfind(params, "#k~", true) != -1)
		return SendClientMessage(playerid, -1, "ERROR: The constant key is not allowed in announce.");
	if(strfind(params, "/q", true) != -1)
		return SendClientMessage(playerid, -1, "ERROR: You are not allowed to type /q in announcement!");
	...
	if(iTemp % 2 == 1)
		return SendClientMessage(playerid, -1, "ERROR: You either have an extra ~ or one is missing in the announcement!");

	foreach(new pid : Player)
	{
		ShowAnnouncement(pid, AccountData[playerid][pAdminname], params);
	}
...
CMD:oban(playerid, params[])
{
	...
	new query[200];
	mysql_format(mdb_query, query, sizeof(query), "SELECT Char_IP FROM player_characters WHERE Char_Name='%e'", player);
	mysql_tquery(mdb_query, query, "OnOBanQueryData", "dssd", playerid, player, reason, datez);
	return 1;
}
...
CMD:reco(playerid, params[])
{
	...
	new shstr[125];
	mysql_format(mdb_query, shstr, sizeof(shstr), "UPDATE `player_characters` SET `Char_BankMoney`=0, `Char_Money`=0 WHERE `pID`=%d", AccountData[userid][pID]);
	mysql_tquery(mdb_query, shstr);
...
CMD:giveweap(playerid, params[])
{
	...
	new otherid;
    if(AccountData[playerid][pAdmin] < 4 || AccountData[playerid][pAdminBS] < 1)
        return PermissionError(playerid);
    ...
    if(sscanf(params, "udI(250)d", otherid, weaponid, ammo, type))
        return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/givewep [playerid/PartOfName] [weaponid] [ammo] [type 0. Normal, 1. Surplus]");
	...
	if(type < 0 || type > 1) return ShowTDN(playerid, NOTIFICATION_ERROR, "Invalid Type");

    GivePlayerWeaponEx(otherid, weaponid, ammo, type);
	...
	static shstr[125];
	format(shstr, sizeof(shstr), "Menggunakan cmd /giveweap %s(%d ammo) type : %s kepada akun dengan nama %s.", ReturnWeaponName(weaponid), ammo, type, AccountData[otherid][pName]);
	AddAdminLog(AccountData[playerid][pName], AccountData[playerid][pUCP], GetStaffRank(playerid), shstr);
	return 1;
}
...
CMD:setadminname(playerid, params[])
{
	...
	new query[125];
	mysql_format(mdb_query, query, sizeof(query), "SELECT `Char_AdminName` FROM `player_characters` WHERE `Char_AdminName`='%s'", aname);
	mysql_tquery(mdb_query, query, "a_ChangeAdminName", "iis", otherid, playerid, aname);
	return 1;
}
...
CMD:setbankmoney(playerid, params[])
{
	...
	mysql_format(mdb_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_BankMoney`=%d WHERE `pID`=%d", value, AccountData[otherid][pID]);
	mysql_tquery(mdb_query, tss);
	return 1;
}
...
CMD:setadmin(playerid, params[])
{
	...
	if(!IsPlayerConnected(otherid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Pemain tersebut tidak terkoneksi ke server!");
	if(value < 0 || value > 10) return ShowTDN(playerid, NOTIFICATION_ERROR, "Level tidak valid! (0-10)");
	...
	mysql_format(mdb_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_Admin`=%d WHERE `pID`=%d", value, AccountData[otherid][pID]);
	mysql_tquery(mdb_query, tss);
	return 1;
}
...
CMD:checkucp(playerid, params[])
{
	...
	new query[596];
	mysql_format(mdb_query, query, sizeof(query), "SELECT `Char_Name` FROM `player_characters` WHERE `Char_UCP`='%e'", name);
	mysql_tquery(mdb_query, query, "CheckUCP", "ds", playerid, name);
	return 1;
}
CMD:savepos(playerid, params[])
{
    ...
    fwrite(fhandle, msg);
    fclose(fhandle);
	Info(playerid, "Coordinat Posisi Kamu Berhasil Di Save!");
...
public OfflineStats(playerid, const name[])
{
	...
	else 
	{
		...
		mysql_format(mdb_query, query, sizeof(query), "SELECT * FROM `families` WHERE `F_ID`=%d", Fams);
		CheckFamilies = mysql_query(mdb_query, query);
