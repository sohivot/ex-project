...
public DCC_DM_EMBED(str[], pin, id[])
{
	...
	mysql_format(mdb_query, query, sizeof query, "INSERT INTO `playerucp` (`ucp`, `verifycode`, `DiscordID`) VALUES ('%e', '%d', '%e')", str, pin, id);
...
public DCC_DM_LUPAPW(pin, id[])
{
	...
	mysql_tquery(mdb_query, sprintf("UPDATE playerucp SET verifycode='%d', password='', salt='' WHERE DiscordID='%s'", pin, id));
	// mysql_format(mdb_query, query, sizeof query, "INSERT INTO `playerucp` (`ucp`, `verifycode`, `DiscordID`) VALUES ('%e', '%d', '%e')", str, pin, id);
	// mysql_tquery(mdb_query, query);
	return 1;
}
...
public CheckAccountUCP(DiscordID[])
{
	...
	//mysql_tquery(mdb_query, sprintf("UPDATE playerucp SET `password`='' WHERE DiscordID = '%s'", DiscordID));
	return 1;
}
...
public CheckDiscordPlayer(DiscordID[])
{
	...
	{
		new query[200];
		mysql_format(mdb_query, query, sizeof(query), "SELECT `ucp` FROM `playerucp` WHERE `DiscordID` = '%e'", DiscordID);
		mysql_tquery(mdb_query, query, "CheckAccountUCP", "s", DiscordID);
	}
	return 1;
}
...
public CheckDiscordID(DiscordID[], Nama_UCP[])
{
	...
	{
		new query[200];
		mysql_format(mdb_query, query, sizeof(query), "SELECT * FROM `playerucp` WHERE `ucp`='%s'", Nama_UCP);
		mysql_tquery(mdb_query, query, "CheckDiscordUCP", "ss", DiscordID, Nama_UCP);
	}
	return 1;
}
...
DCMD:resetpassword(user, channel, params[])
{
	...
	mysql_format(mdb_query, characterQuery, sizeof(characterQuery), "SELECT * FROM `playerucp` WHERE `DiscordID` = '%s'", id);
	mysql_tquery(mdb_query, characterQuery, "CheckDiscordPlayer", "ss", id, params);
	return 1;
}
*/
DCMD:restart(user, channel, params[])
{
	if(channel != DCC_FindChannelById("1302140466439127050"))
		return 1;
...
DCMD:ambiltiket(user, channel, params[])
{
	...
	new cQuery[200];
	mysql_format(mdb_query, cQuery, sizeof(cQuery), "SELECT * FROM `playerucp` WHERE `DiscordID`='%s'", userID);
	mysql_tquery(mdb_query, cQuery, "CheckDiscordID", "ss", userID, params);
	return 1;
}